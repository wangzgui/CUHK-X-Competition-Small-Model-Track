...
CFG = dict(
    ...
    architecture_version="dual_stream_structured_dropout_v5",
...
class FactorizedSensorStream(nn.Module):
    ...
    def __init__(self, input_dim, n_classes):
        ...
        self.time_position = nn.Parameter(torch.zeros(1, CFG["steps"] + 1, sd))
    ...
    def forward(self, x, sensor_mask):
        ...
        z = self.temporal_norm(self.temporal_encoder(z)[:, 0])
...
class TinyIMUTransformer(nn.Module):
    ...
    def forward(self, x, sensor_mask, return_branches=False):
        delta = torch.zeros_like(x)
        delta[:, 1:] = x[:, 1:] - x[:, :-1]
        motion = torch.cat([delta, delta.abs()], dim=-1)
        raw_logits = self.raw_stream(x, sensor_mask)
        motion_logits = self.motion_stream(motion, sensor_mask)
