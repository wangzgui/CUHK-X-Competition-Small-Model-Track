"""Native IMU Transformer v8.1 with user-balanced training sampling.
...
Training examples are sampled with inverse user-frequency weights so every
training subject contributes approximately equally within each epoch.
"""
import json
import math
import random
...
OUT_DIR = Path("/kaggle/working/imu_transformer_user_balanced_v81")
...
CFG = dict(
    ...
    architecture_version="dual_stream_user_balanced_v81",
...
def user_balanced_weights(user_values):
    """Return mean-one inverse-frequency weights in dataset row order."""
    users = np.asarray(user_values).astype(str)
    if len(users) == 0:
        raise ValueError("Cannot create user-balanced weights for an empty set")
    unique_users, counts = np.unique(users, return_counts=True)
    inverse_count = {user: 1. / count
                     for user, count in zip(unique_users, counts)}
    weights = np.asarray([inverse_count[user] for user in users], np.float64)
...
def train_model(train_ds, val_ds, n_classes, epochs, seed, fold=None,
    ...
    if sample_weights is not None:
        sample_weights = np.asarray(sample_weights, np.float64)
        if len(sample_weights) != len(train_ds):
            raise ValueError(
                f"Sampler weights={len(sample_weights)} but dataset={len(train_ds)}")
        sampler = WeightedRandomSampler(
            weights=torch.as_tensor(sample_weights, dtype=torch.double),
            num_samples=len(train_ds), replacement=True)
    train_loader = DataLoader(
        train_ds, CFG["batch_size"], shuffle=sampler is None,
        sampler=sampler, drop_last=False, num_workers=CFG["workers"],
        pin_memory=True)
...
def save_final(model, model_id, mean, std, label_values, epochs):
    ...
    torch.save(dict(state_dict=compact_state_dict(model.state_dict()), mean=mean,
                    ...
                    max_len=CFG["max_len"],
                    user_balanced_sampling=CFG["user_balanced_sampling"],
...
def main():
    ...
    user_counts = pd.Series(groups).value_counts().sort_index()
    print("User-balanced sampling:",
          {"enabled": CFG["user_balanced_sampling"],
           "users": len(user_counts),
           "minimum_clips_per_user": int(user_counts.min()),
           "maximum_clips_per_user": int(user_counts.max()),
           "draws_per_epoch": len(index),
           "replacement": True})
    ...
    for fold, (tr, va) in enumerate(splitter.split(index, y, groups)):
        train_weights = (user_balanced_weights(groups[tr])
                         if CFG["user_balanced_sampling"] else None)
        fold_counts = pd.Series(groups[tr]).value_counts()
        weight_text = (f"{train_weights.min():.3f}..{train_weights.max():.3f}"
                       if train_weights is not None else "disabled")
        print(f"fold {fold} sampler: users={len(fold_counts)}, "
              f"clips/user={int(fold_counts.min())}..{int(fold_counts.max())}, "
        ...
        _, score, best_epoch, prob = train_model(
            train_ds, val_ds, len(label_values), CFG["epochs"],
            CFG["seed"] + fold, fold, sample_weights=train_weights)
    ...
    final_weights = (user_balanced_weights(groups)
                     if CFG["user_balanced_sampling"] else None)
    for model_id in range(CFG["final_models"]):
        ...
        model, _, _, _ = train_model(
            train_ds, None, len(label_values), final_epochs,
