"""Time-aware native-sampling IMU Transformer, without interpolation.
...
Two explicit timing features encode relative progress and local sampling gap;
the motion stream also corrects its difference by the normalized time gap.
"""
import json
import math
import random
...
OUT_DIR = Path("/kaggle/working/imu_transformer_time_aware_v9")
...
CFG = dict(
    ...
    architecture_version="dual_stream_native_time_aware_v9",
...
TIME_FEATURE_DIM = 2  # relative_time, log(dt / per-sequence median_dt)
...
def native_rows(part):
    """Return native values, valid mask, and explicit timing features."""
    ...
    if len(values) > CFG["max_len"]:
        ...
        times = times[start:start + CFG["max_len"]]
    output = np.zeros((CFG["max_len"], values.shape[1]), np.float32)
    valid = np.zeros(CFG["max_len"], bool)
    time_features = np.zeros((CFG["max_len"], TIME_FEATURE_DIM), np.float32)
    output[:len(values)] = values.astype(np.float32)
    valid[:len(values)] = True
    if len(times):
        duration = times[-1] - times[0]
        relative_time = ((times - times[0]) / duration
                         if duration > 1e-9 else np.zeros(len(times)))
        gaps = np.zeros(len(times), np.float64)
        if len(times) > 1:
            positive_gaps = np.diff(times)
            median_gap = np.median(positive_gaps[positive_gaps > 0])
            if np.isfinite(median_gap) and median_gap > 1e-9:
                gap_ratio = np.clip(positive_gaps / median_gap, .25, 4.)
                gaps[1:] = np.log(gap_ratio)
        time_features[:len(times), 0] = relative_time.astype(np.float32)
        time_features[:len(times), 1] = gaps.astype(np.float32)
    return output, valid, time_features
...
def read_imu_pair(clip_dir):
    """Return native rows plus sensor, temporal, and timing-feature masks."""
    frames = []
    ...
    time_features = np.zeros(
        (CFG["max_len"], len(SENSORS), TIME_FEATURE_DIM), np.float32)
    if not frames:
        return output, mask, time_mask, time_features
    ...
    for sensor, sensor_idx in SENSOR_TO_INDEX.items():
        ...
        if part.empty:
            continue
        (output[:, sensor_idx], time_mask[:, sensor_idx],
         time_features[:, sensor_idx]) = native_rows(part)
        mask[sensor_idx] = 1.
    return (np.nan_to_num(output, nan=0., posinf=0., neginf=0.),
            mask, time_mask, time_features)
...
def build_cache(index, tag, clip_dirs):
    ...
    x_path = OUT_DIR / f"{tag}_native_time_v9_l{CFG['max_len']}_f{fdim}.npy"
    m_path = OUT_DIR / f"{tag}_sensor_mask_v9_l{CFG['max_len']}.npy"
    tm_path = OUT_DIR / f"{tag}_time_mask_v9_l{CFG['max_len']}.npy"
    tf_path = OUT_DIR / f"{tag}_time_features_v9_l{CFG['max_len']}.npy"
    ...
    expected_feature_shape = (*expected_time_shape, TIME_FEATURE_DIM)
    if (x_path.exists() and m_path.exists() and tm_path.exists() and
            tf_path.exists()):
        try:
            cached_x = np.load(x_path, mmap_mode="r")
            cached_mask = np.load(m_path, mmap_mode="r")
            cached_time = np.load(tm_path, mmap_mode="r")
            cached_time_features = np.load(tf_path, mmap_mode="r")
            if (cached_x.shape == expected_shape and
                    cached_mask.shape == expected_mask_shape and
                    cached_time.shape == expected_time_shape and
                    cached_time_features.shape == expected_feature_shape):
                return (cached_x, cached_mask, cached_time,
                        cached_time_features)
...
class IMUDataset(Dataset):
    def __init__(self, x, mask, time_mask, time_features, indices,
                 mean, std, y=None, train=False):
        self.x, self.mask = x, mask
        self.time_mask, self.time_features = time_mask, time_features
    ...
    def __getitem__(self, item):
        ...
        time_features = np.asarray(self.time_features[source_idx]).copy()
        ...
        return (torch.from_numpy(x.astype(np.float32)),
                ...
                torch.from_numpy(time_features.astype(np.float32)), label)
...
class FactorizedSensorStream(nn.Module):
    ...
    def __init__(self, input_dim, n_classes):
        ...
        # Keep timing metadata separate from physical channels so fold-specific
        # IMU normalization cannot distort relative progress or sampling gaps.
        self.time_proj = nn.Linear(TIME_FEATURE_DIM, sd, bias=False)
    ...
    def forward(self, x, sensor_mask, time_mask, time_features):
        b, t, s, _ = x.shape
        z = self.sensor_proj(x) + self.time_proj(time_features)
        z = z * time_mask[..., None]
...
class TinyIMUTransformer(nn.Module):
    ...
    def forward(self, x, sensor_mask, time_mask, time_features,
        ...
        # log_dt_ratio is clipped during extraction. Correct differences back
        # to one typical sample interval without producing extreme derivatives.
        dt_ratio = torch.exp(time_features[..., 1]).clamp(.5, 2.)
        delta[:, 1:] = ((x[:, 1:] - x[:, :-1]) /
                        dt_ratio[:, 1:, :, None])
        adjacent = time_mask[:, 1:] & time_mask[:, :-1]
        delta[:, 1:] *= adjacent[..., None]
        motion = torch.cat([delta, delta.abs()], dim=-1)
        raw_logits = self.raw_stream(
            x, sensor_mask, time_mask, time_features)
        motion_logits = self.motion_stream(
            motion, sensor_mask, time_mask, time_features)
...
def predict(model, loader):
    ...
    for x, mask, time_mask, time_features, _ in loader:
        ...
        probabilities.append(torch.softmax(
            model(x, mask, time_mask, time_features), 1).cpu().numpy())
...
def train_model(train_ds, val_ds, n_classes, epochs, seed, fold=None):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x, mask, time_mask, time_features, y in train_loader:
            x = x.to(DEVICE, non_blocking=True)
            mask = mask.to(DEVICE, non_blocking=True)
            time_mask = time_mask.to(DEVICE, non_blocking=True)
            time_features = time_features.to(DEVICE, non_blocking=True)
            ...
            with autocast(enabled=scaler.is_enabled()):
                fused_logits, raw_logits, motion_logits = model(
                    x, mask, time_mask, time_features,
...
def save_final(model, model_id, mean, std, label_values, epochs):
    ...
    torch.save(dict(state_dict=compact_state_dict(model.state_dict()), mean=mean,
                    ...
                    max_len=CFG["max_len"], time_feature_dim=TIME_FEATURE_DIM,
...
def main():
    ...
    (train_x, train_mask, train_time_mask,
     train_time_features) = build_cache(
    ...
    (test_x, test_mask, test_time_mask,
     test_time_features) = build_cache(
    ...
    for fold, (tr, va) in enumerate(splitter.split(index, y, groups)):
        ...
        train_ds = IMUDataset(
            train_x, train_mask, train_time_mask, train_time_features,
            tr, mean, std, y, True)
        val_ds = IMUDataset(
            train_x, train_mask, train_time_mask, train_time_features,
            va, mean, std, y, False)
    ...
    test_ds = IMUDataset(
        test_x, test_mask, test_time_mask, test_time_features, test_indices,
        mean, std, None, False)
