"""Native-sampling IMU Transformer with quaternion sign continuity.
...
Quaternion signs are made temporally continuous before motion differences are
formed, preventing equivalent q/-q representations from creating false peaks.
"""
import json
import math
import random
...
OUT_DIR = Path("/kaggle/working/imu_transformer_quaternion_continuity_v12")
...
CFG = dict(
    ...
    architecture_version="dual_stream_quaternion_continuity_v12",
    cache_version="qcontinuity_v1",
...
QUAT_FEATURE_SLICE = slice(12, 16)
...
def stabilize_quaternion_signs(values):
    """Normalize quaternions and choose a continuous q/-q representation.

    Quaternion component order and rotation convention do not affect this
    operation: q and -q always describe the same orientation. Zero/invalid
    quaternion rows are retained and do not reset the last valid orientation.
    """
    if len(values) == 0:
        return values
    quat = values[:, QUAT_FEATURE_SLICE].copy()
    norms = np.linalg.norm(quat, axis=1)
    valid = np.isfinite(quat).all(axis=1) & (norms > 1e-6)
    quat[valid] /= norms[valid, None]
    previous = None
    for row in np.flatnonzero(valid):
        if previous is not None and np.dot(quat[row], previous) < 0.:
            quat[row] *= -1.
        previous = quat[row].copy()
    quat[~valid] = 0.
    values[:, QUAT_FEATURE_SLICE] = quat
    return values
...
def native_rows(part):
    ...
    # Align signs before averaging duplicate timestamps so equivalent q/-q
    # samples cannot cancel each other into a near-zero quaternion.
    values = stabilize_quaternion_signs(values)
    ...
    # Averaging can move a quaternion away from unit length. Renormalize and
    # enforce continuity again on the final unique-timestamp sequence.
    values = stabilize_quaternion_signs(values)
...
def build_cache(index, tag, clip_dirs):
    ...
    cache = CFG["cache_version"]
    x_path = OUT_DIR / f"{tag}_native_l{CFG['max_len']}_f{fdim}_{cache}.npy"
    m_path = OUT_DIR / f"{tag}_sensor_mask_l{CFG['max_len']}_{cache}.npy"
    tm_path = OUT_DIR / f"{tag}_time_mask_l{CFG['max_len']}_{cache}.npy"
