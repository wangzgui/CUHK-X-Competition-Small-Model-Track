"""Lightweight five-sensor IMU Transformer for 40-class action recognition.

The two CSV files of one clip contain interleaved rows from five devices. Each
device is sorted independently and interpolated to 64 normalized time steps.
The script performs subject-disjoint 5-fold CV, trains two full-data models,
saves OOF/test probabilities for later fusion, and writes imu_submission.csv.
"""
import json
import math
import random
...
OUT_DIR = Path("/kaggle/working/imu_transformer")
SUBMISSION_PATH = Path("/kaggle/working/submission.csv")

CFG = dict(
    seed=2026, steps=64, folds=5, epochs=50, warmup_epochs=4,
    batch_size=96, workers=2, lr=1.5e-3, min_lr=1e-5,
    weight_decay=5e-4, label_smoothing=.05, amp=True,
    model_dim=112, sensor_dim=24, heads=4, layers=3, ff_dim=224,
...
def interpolate_rows(values, times, n_out):
    """Sort, merge duplicate timestamps, then interpolate on normalized time."""
    if len(values) == 0:
        return np.zeros((n_out, values.shape[1]), np.float32)
    ...
    if len(values) == 0:
        return np.zeros((n_out, values.shape[1]), np.float32)
    # Aggregate duplicate timestamps rather than passing repeated x to np.interp.
    unique_t, inverse = np.unique(times, return_inverse=True)
    if len(unique_t) != len(times):
        merged = np.zeros((len(unique_t), values.shape[1]), np.float64)
        count = np.zeros(len(unique_t), np.float64)
        np.add.at(merged, inverse, values); np.add.at(count, inverse, 1)
        values, times = merged / count[:, None], unique_t
    if len(values) == 1 or times[-1] <= times[0]:
        return np.repeat(values[:1], n_out, axis=0).astype(np.float32)
    source = (times - times[0]) / (times[-1] - times[0])
    target = np.linspace(0., 1., n_out)
    return np.stack([np.interp(target, source, values[:, j])
                     for j in range(values.shape[1])], axis=1).astype(np.float32)


def transform_numeric(frame):
    """Convert raw columns to stable physical features; omit device leakage."""
    def values(columns):
        return frame[columns].apply(pd.to_numeric, errors="coerce").interpolate(
            limit_direction="both").fillna(0).to_numpy(np.float64)
...
def read_imu_pair(clip_dir):
    """Return x=(64,5,F), sensor mask=(5,), and duration statistics."""
    frames = []
    ...
    output = np.zeros((CFG["steps"], len(SENSORS), feature_dim), np.float32)
    mask = np.zeros(len(SENSORS), np.float32)
    if not frames:
        return output, mask
    data = pd.concat(frames, ignore_index=True)
    data["_sensor"] = data["设备名称"].map(device_prefix)
    parsed_time = pd.to_datetime(data["时间"], errors="coerce")
    # Nanoseconds are converted to seconds; absolute date is never a feature.
    seconds = parsed_time.astype("int64").to_numpy(np.float64) / 1e9
    seconds[parsed_time.isna().to_numpy()] = np.nan
    data["_seconds"] = seconds
    for sensor, sensor_idx in SENSOR_TO_INDEX.items():
        part = data.loc[data["_sensor"] == sensor].copy()
        if part.empty:
            continue
        raw = transform_numeric(part)
        times = pd.to_numeric(part["_seconds"], errors="coerce").to_numpy(np.float64)
        if not np.isfinite(times).any():
            times = np.arange(len(part), dtype=np.float64)
        else:
            times = np.where(np.isfinite(times), times, np.nanmedian(times))
        output[:, sensor_idx] = interpolate_rows(raw, times, CFG["steps"])
        mask[sensor_idx] = 1.
    return np.nan_to_num(output, nan=0., posinf=0., neginf=0.), mask
...
def build_cache(index, tag, clip_dirs):
    ...
    x_path = OUT_DIR / f"{tag}_raw_t{CFG['steps']}_f{fdim}.npy"
    m_path = OUT_DIR / f"{tag}_mask_t{CFG['steps']}_f{fdim}.npy"
    expected_shape = (len(clip_dirs), CFG["steps"], len(SENSORS), fdim)
    expected_mask_shape = (len(clip_dirs), len(SENSORS))
    if x_path.exists() and m_path.exists():
        try:
            cached_x, cached_mask = np.load(x_path, mmap_mode="r"), np.load(m_path, mmap_mode="r")
            if cached_x.shape == expected_shape and cached_mask.shape == expected_mask_shape:
                return cached_x, cached_mask
        except (OSError, ValueError):
            pass
    x = np.empty((len(clip_dirs), CFG["steps"], len(SENSORS), fdim), np.float32)
    mask = np.empty((len(clip_dirs), len(SENSORS)), np.float32)
    for i, directory in enumerate(tqdm(clip_dirs, desc=f"Extract {tag} IMU")):
        x[i], mask[i] = read_imu_pair(Path(directory))
    np.save(x_path, x); np.save(m_path, mask)
    return np.load(x_path, mmap_mode="r"), np.load(m_path, mmap_mode="r")


def compute_stats(x, mask, indices):
    ...
    for start in range(0, len(indices), 256):
        ...
        present = np.asarray(mask[ids], dtype=np.float64)[:, None, :, None]
        total += (batch * present).sum(axis=(0, 1))
        square += (batch * batch * present).sum(axis=(0, 1))
        count += present.sum(axis=(0, 1)) * CFG["steps"]
...
class IMUDataset(Dataset):
    def __init__(self, x, mask, indices, mean, std, y=None, train=False):
        self.x, self.mask = x, mask
    ...
    def __getitem__(self, item):
        ...
        if self.train:
            ...
            if random.random() < .4:
                x += np.random.normal(0, .025, x.shape).astype(np.float32) * sensor_mask[None, :, None]
            if random.random() < .15 and sensor_mask.sum() > 3:
                drop = random.choice(np.flatnonzero(sensor_mask).tolist())
                sensor_mask[drop] = 0.; x[:, drop] = 0.
        label = -1 if self.y is None else int(self.y[source_idx])
        return torch.from_numpy(x.astype(np.float32)), torch.from_numpy(sensor_mask.astype(np.float32)), label
...
class TinyIMUTransformer(nn.Module):
    def __init__(self, n_classes, feature_dim):
        ...
        self.sensor_proj = nn.Sequential(nn.Linear(feature_dim, sd), nn.LayerNorm(sd), nn.GELU())
        self.sensor_embed = nn.Parameter(torch.zeros(1, 1, len(SENSORS), sd))
        self.fusion = nn.Sequential(nn.Linear(len(SENSORS) * sd, d), nn.LayerNorm(d), nn.GELU())
        self.cls = nn.Parameter(torch.zeros(1, 1, d))
        self.position = nn.Parameter(torch.zeros(1, CFG["steps"] + 1, d))
    ...
    def forward(self, x, sensor_mask):
        # x: (B,T,5,F). Masked sensors remain zero after adding embeddings.
        b, t, s, _ = x.shape
        z = self.sensor_proj(x) + self.sensor_embed[:, :, :s]
        z *= sensor_mask[:, None, :, None]
        z = self.fusion(z.reshape(b, t, -1))
        cls = self.cls.expand(b, -1, -1)
        z = torch.cat([cls, z], dim=1) + self.position[:, :t + 1]
        return self.head(self.norm(self.encoder(z)[:, 0]))
...
def predict(model, loader):
    ...
    for x, mask, _ in loader:
        ...
        probabilities.append(torch.softmax(model(x, mask), 1).cpu().numpy())
...
def train_model(train_ds, val_ds, n_classes, epochs, seed, fold=None):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x, mask, y in train_loader:
            x, mask, y = x.to(DEVICE, non_blocking=True), mask.to(DEVICE, non_blocking=True), y.to(DEVICE, non_blocking=True)
            lr = scheduled_lr(step, total_steps, warmup_steps)
            for group in optimizer.param_groups: group["lr"] = lr
            optimizer.zero_grad(set_to_none=True)
            with autocast(enabled=scaler.is_enabled()):
                loss = F.cross_entropy(model(x, mask), y, label_smoothing=CFG["label_smoothing"])
        ...
        if val_loader is not None:
            ...
            print(f"fold {fold} epoch {epoch:02d}/{epochs}: loss={loss_sum/max(seen,1):.4f} val={accuracy:.4f} best={best_acc:.4f}")
        else:
            print(f"final seed {seed} epoch {epoch:02d}/{epochs}: loss={loss_sum/max(seen,1):.4f}")
...
def save_final(model, model_id, mean, std, label_values, epochs):
    ...
    torch.save(dict(state_dict=compact_state_dict(model.state_dict()), mean=mean,
                    ...
                    sensors=SENSORS, feature_dim=numeric_feature_dim(), epochs=epochs), path)
...
def main():
    ...
    train_x, train_mask = build_cache(index, "train", index.clip_dir.tolist())
    ...
    test_x, test_mask = build_cache(test, "test", test_clip_dirs(test))
    ...
    print(f"IMU Transformer parameters: {params:,}; estimated two-model FP16 weights: {params*2*CFG['final_models']/1024**2:.3f} MB")
    del probe
    ...
    for fold, (tr, va) in enumerate(splitter.split(index, y, groups)):
        mean, std = compute_stats(train_x, train_mask, tr)
        train_ds = IMUDataset(train_x, train_mask, tr, mean, std, y, True)
        val_ds = IMUDataset(train_x, train_mask, va, mean, std, y, False)
    ...
    mean, std = compute_stats(train_x, train_mask, all_indices)
    test_ds = IMUDataset(test_x, test_mask, test_indices, mean, std, None, False)
    ...
    for model_id in range(CFG["final_models"]):
        train_ds = IMUDataset(train_x, train_mask, all_indices, mean, std, y, True)
