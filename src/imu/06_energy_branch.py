"""Native IMU Transformer with length alignment and invariant energy branch.
...
Training clips receive a conservative native contiguous crop to reduce the
audited train/test length shift without synthesizing or interpolating samples.
An independent lightweight branch models rotation-invariant acceleration and
gyroscope magnitudes with multi-scale depthwise temporal convolutions.
"""
import json
import math
import random
...
OUT_DIR = Path("/kaggle/working/imu_transformer_energy_branch_v10")
...
CFG = dict(
    ...
    architecture_version="dual_stream_invariant_energy_v10",
    auxiliary_loss_weight=.30, initial_motion_weight=.65,
    energy_auxiliary_loss_weight=.20, initial_energy_weight=.15,
    energy_hidden=32, energy_model_dim=64,
    paired_leg_dropout=.09, single_sensor_dropout=.10,
    native_crop_probability=.35, native_crop_min_ratio=.75,
    native_crop_max_ratio=1.0, native_crop_center_probability=.70,
    native_crop_min_points=8,
...
def native_length_crop(x, time_mask, sensor_mask):
    """Crop one continuous native segment per present sensor, then repad.

    A single ratio and relative random start are shared across devices. This
    retains approximately synchronized action regions even when their native
    row counts differ by a few samples.
    """
    ratio = random.uniform(
        CFG["native_crop_min_ratio"], CFG["native_crop_max_ratio"])
    centered = random.random() < CFG["native_crop_center_probability"]
    relative_start = random.random()
    for sensor in range(len(SENSORS)):
        if sensor_mask[sensor] <= 0:
            continue
        valid_indices = np.flatnonzero(time_mask[:, sensor])
        n = len(valid_indices)
        if n <= 1:
            continue
        keep = min(n, max(CFG["native_crop_min_points"],
                          int(round(n * ratio))))
        if keep >= n:
            continue
        available = n - keep
        start = available // 2 if centered else int(round(relative_start * available))
        # Native caches are sorted and compacted to the left. Copy before
        # clearing to avoid overlapping-assignment aliasing.
        selected = x[start:start + keep, sensor].copy()
        x[:, sensor] = 0.
        time_mask[:, sensor] = False
        x[:keep, sensor] = selected
        time_mask[:keep, sensor] = True
    return x, time_mask
...
class IMUDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if (self.train and
                random.random() < CFG["native_crop_probability"]):
            x, time_mask = native_length_crop(x, time_mask, sensor_mask)
...
class InvariantEnergyStream(nn.Module):
    """Rotation-invariant magnitude dynamics with lightweight temporal CNNs."""
    def __init__(self, n_classes):
        super().__init__()
        channels = 8  # magnitudes, centered magnitudes, delta, abs(delta)
        hidden, d = CFG["energy_hidden"], CFG["energy_model_dim"]

        def temporal_branch(kernel, dilation):
        ...
        self.local = temporal_branch(3, 1)
        self.context = temporal_branch(3, 3)  # effective receptive field 7
        self.merge = nn.Sequential(
            nn.Conv1d(hidden * 2, hidden, 1, bias=False),
            nn.GroupNorm(4, hidden), nn.GELU(),
        )
        self.sensor_up = nn.Sequential(
            nn.Linear(hidden * 2, d), nn.LayerNorm(d), nn.GELU())
        self.sensor_embed = nn.Parameter(torch.zeros(1, len(SENSORS), d))
        self.global_cls = nn.Parameter(torch.zeros(1, 1, d))
        fusion_layer = nn.TransformerEncoderLayer(
        ...
        self.fusion = nn.TransformerEncoder(fusion_layer, 1)
        self.norm = nn.LayerNorm(d)
        self.head = nn.Sequential(
            nn.Dropout(CFG["dropout"]), nn.Linear(d, n_classes))
        nn.init.trunc_normal_(self.sensor_embed, std=.02)
        nn.init.trunc_normal_(self.global_cls, std=.02)

    @staticmethod
    def make_energy_features(x, time_mask):
        # make_features stores acc_norm and gyro_norm as channels 16 and 17.
        magnitude = x[..., 16:18]
        valid = time_mask[..., None].to(magnitude.dtype)
        count = valid.sum(dim=1, keepdim=True).clamp_min(1.)
        center = (magnitude * valid).sum(dim=1, keepdim=True) / count
        centered = (magnitude - center) * valid
        delta = torch.zeros_like(magnitude)
        adjacent = time_mask[:, 1:] & time_mask[:, :-1]
        delta[:, 1:] = ((magnitude[:, 1:] - magnitude[:, :-1]) *
                        adjacent[..., None])
        return torch.cat([
            magnitude * valid, centered, delta, delta.abs()], dim=-1)

    def forward(self, x, sensor_mask, time_mask):
        b, t, s, _ = x.shape
        energy = self.make_energy_features(x, time_mask)
        energy = energy.permute(0, 2, 3, 1).reshape(b * s, 8, t)
        valid = time_mask.permute(0, 2, 1).reshape(b * s, 1, t)
        z = torch.cat([self.local(energy), self.context(energy)], dim=1)
        z = self.merge(z) * valid.to(z.dtype)

        denominator = valid.sum(dim=-1).clamp_min(1).to(z.dtype)
        mean_pool = z.sum(dim=-1) / denominator
        lowest = torch.finfo(z.dtype).min
        max_pool = z.masked_fill(~valid, lowest).amax(dim=-1)
        present = sensor_mask.reshape(b * s, 1) > 0
        max_pool = torch.where(present, max_pool, torch.zeros_like(max_pool))
        token = self.sensor_up(torch.cat([mean_pool, max_pool], dim=-1))
        token = token.reshape(b, s, -1) + self.sensor_embed[:, :s]

        global_cls = self.global_cls.expand(b, -1, -1)
        token = torch.cat([global_cls, token], dim=1)
        padding_mask = torch.cat([
            torch.zeros((b, 1), dtype=torch.bool, device=x.device),
            sensor_mask[:, :s] <= 0,
        ], dim=1)
        token = self.fusion(token, src_key_padding_mask=padding_mask)
        return self.head(self.norm(token[:, 0]))


class TinyIMUTransformer(nn.Module):
    """Raw, motion, and invariant-energy streams fused at classifier logits."""
    def __init__(self, n_classes, feature_dim):
        ...
        self.energy_stream = InvariantEnergyStream(n_classes)
        ...
        energy_initial = float(CFG["initial_energy_weight"])
        self.energy_weight_logit = nn.Parameter(torch.tensor(
            math.log(energy_initial / (1. - energy_initial)),
            dtype=torch.float32))
    ...
    def forward(self, x, sensor_mask, time_mask, return_branches=False):
        ...
        energy_logits = self.energy_stream(x, sensor_mask, time_mask)
        motion_weight = torch.sigmoid(self.motion_weight_logit)
        base_logits = ((1. - motion_weight) * raw_logits +
                       motion_weight * motion_logits)
        energy_weight = torch.sigmoid(self.energy_weight_logit)
        fused_logits = ((1. - energy_weight) * base_logits +
                        energy_weight * energy_logits)
        if return_branches:
            return fused_logits, raw_logits, motion_logits, energy_logits
        return fused_logits
...
def train_model(train_ds, val_ds, n_classes, epochs, seed, fold=None):
    ...
    for epoch in range(1, epochs + 1):
        model.train()
        loss_sum = fused_sum = raw_sum = motion_sum = energy_sum = 0.
        seen = 0
        for x, mask, time_mask, y in train_loader:
            ...
            with autocast(enabled=scaler.is_enabled()):
                fused_logits, raw_logits, motion_logits, energy_logits = model(
                ...
                energy_loss = F.cross_entropy(
                    energy_logits, y, label_smoothing=CFG["label_smoothing"])
                loss = (fused_loss + CFG["auxiliary_loss_weight"] *
                        (raw_loss + motion_loss) +
                        CFG["energy_auxiliary_loss_weight"] * energy_loss)
            ...
            energy_sum += energy_loss.item() * len(y)
            seen += len(y)
        if val_loader is not None:
            ...
            motion_weight = torch.sigmoid(
                model.motion_weight_logit.detach()).item()
            energy_weight = torch.sigmoid(
                model.energy_weight_logit.detach()).item()
            print(f"fold {fold} epoch {epoch:02d}/{epochs}: "
                  ...
                  f"energy={energy_sum/max(seen,1):.4f} "
                  f"motion_w={motion_weight:.3f} energy_w={energy_weight:.3f} "
                  f"val={accuracy:.4f} best={best_acc:.4f}")
        else:
            motion_weight = torch.sigmoid(
                model.motion_weight_logit.detach()).item()
            energy_weight = torch.sigmoid(
    ...
    energy_weight = torch.sigmoid(model.energy_weight_logit.detach()).item()
    prefix = "final" if fold is None else f"fold {fold}"
    print(f"{prefix}: learned late-fusion weights "
          f"motion={motion_weight:.3f}, energy={energy_weight:.3f}")
...
def main():
    ...
    print("Native length alignment:",
          {"probability": f"{CFG['native_crop_probability']:.1%}",
           "ratio": (CFG["native_crop_min_ratio"],
                     CFG["native_crop_max_ratio"]),
           "center_probability": f"{CFG['native_crop_center_probability']:.1%}",
           "minimum_points": CFG["native_crop_min_points"]})
    print("Invariant energy branch:",
          {"features": "acc/gyro magnitude + centered + delta + abs(delta)",
           "hidden": CFG["energy_hidden"],
           "model_dim": CFG["energy_model_dim"],
           "initial_weight": CFG["initial_energy_weight"],
           "auxiliary_loss_weight": CFG["energy_auxiliary_loss_weight"]})
