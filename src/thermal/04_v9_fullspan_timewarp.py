...
class Config:
    ...
    fullspan_timewarp_p: float = 0.30
    timewarp_midpoint_range: tuple[float, float] = (0.40, 0.60)
    boundary_users: tuple[str, ...] = ("user8", "user9", "user23", "user24")
    tta_hflip: bool = True
    qa_max_clips: int = 8
    champion_artifact_dir: Path = Path('/kaggle/working/thermal_v8_mvitv2s_cache64_sample32')
    ...
    @property
    def out_dir(self):
        return self.work_root / "thermal_v9_mvitv2s_fullspan_timewarp"


CFG = Config()
assert (CFG.cache_frames, CFG.model_frames, CFG.img_size, CFG.batch_size, CFG.grad_accum) == (64, 32, 128, 2, 6)
assert CFG.tta_hflip and CFG.fullspan_timewarp_p == 0.30
assert CFG.timewarp_midpoint_range == (0.40, 0.60)
...
print('V9: 70% V8 ordered-pair sampling + 30% endpoint-preserving full-span time warp')
print('Time warp: monotonic piecewise-linear map, midpoint in [0.40, 0.60], endpoints exactly 0 and 63.')
print('Validation/test: unchanged single uniform32 + horizontal flip only (2 forwards/sample).')
...
def build_cache(clips, windows, split, cfg):
    ...
    signature_data = dict(version='v8_uniform64', shape=shape,
...
def fullspan_timewarp_indices(midpoint):
    """Map output time 0.5 to `midpoint`, preserving order and both endpoints."""
    lo, hi = CFG.timewarp_midpoint_range
    midpoint = float(midpoint)
    assert lo <= midpoint <= hi
    output_time = np.linspace(0.0, 1.0, CFG.model_frames)
    source_time = np.where(
        output_time <= 0.5,
        output_time * (2.0 * midpoint),
        midpoint + (output_time - 0.5) * (2.0 * (1.0 - midpoint)),
    )
    indices = np.rint(source_time * (CFG.cache_frames - 1)).astype(np.int64)
    indices[0], indices[-1] = 0, CFG.cache_frames - 1
    assert np.all(np.diff(indices) > 0), indices
    return indices


def temporal_indices64(view='single', train=False, rng=None):
    if train:
        rng = np.random if rng is None else rng
        if float(rng.random()) < CFG.fullspan_timewarp_p:
            midpoint = float(rng.uniform(*CFG.timewarp_midpoint_range))
            return fullspan_timewarp_indices(midpoint)
        # V8 branch: one frame from each ordered pair, never shuffle chronology.
        binary = (rng.integers(0, 2, size=32) if hasattr(rng, 'integers')
                  else rng.randint(0, 2, size=32))
        return np.arange(0, 64, 2, dtype=np.int64) + binary
    if view == 'single':
...
def timewarp_policy_audit(cfg, draws=10000):
    """Use an isolated RNG so auditing cannot perturb model training randomness."""
    rng = np.random.default_rng(cfg.seed + 909)
    warped, midpoint_values = 0, []
    schedules = []
    for _ in range(draws):
        use_warp = float(rng.random()) < cfg.fullspan_timewarp_p
        if use_warp:
            midpoint = float(rng.uniform(*cfg.timewarp_midpoint_range))
            indices = fullspan_timewarp_indices(midpoint)
            warped += 1
            midpoint_values.append(midpoint)
        else:
            indices = np.arange(0, 64, 2, dtype=np.int64) + rng.integers(0, 2, size=32)
        schedules.append(indices)
    schedules = np.asarray(schedules)
    report = dict(draws=draws, configured_probability=cfg.fullspan_timewarp_p,
                  observed_probability=warped / draws,
                  midpoint_min=float(np.min(midpoint_values)),
                  midpoint_max=float(np.max(midpoint_values)),
                  every_schedule_chronological=bool(np.all(np.diff(schedules, axis=1) > 0)),
                  warped_endpoints_always_preserved=True)
    (cfg.out_dir / 'timewarp_policy_audit.json').write_text(json.dumps(report, indent=2), encoding='utf-8')
    print('V9 time-warp policy audit:', report)
    return report


sampling_audit(train_clips, CFG, 'train')
sampling_audit(test_clips, CFG, 'test')
timewarp_policy_audit(CFG)
...
class ThermalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Train: 70% V8 pair sampling, 30% endpoint-preserving warp.
        # Validation/test: deterministic full-span schedule, unchanged from V8.
...
@torch.no_grad()
def infer_view_logits(model, meta, cfg, split):
    """V9 inference is deliberately fixed to uniform32 original + hflip."""
    model.eval()
    dataset = ThermalDataset(cfg.cache_dir / f'{split}_thermal.npy', meta, cfg, False, view='single')
    loader = DataLoader(dataset, cfg.batch_size, shuffle=False, **common_loader_args(cfg))
    records = {f'single_{kind}': np.zeros((len(meta), N_CLASSES), np.float32)
               for kind in ('original', 'flipped', 'hflip')}
    for clip, _, idx in tqdm(loader, desc=f'{split}: uniform32 original + hflip'):
        clip = clip.to(DEVICE, non_blocking=True)
        with torch.autocast('cuda', enabled=cfg.amp and DEVICE.type == 'cuda'):
            original = model(clip.permute(0, 2, 1, 3, 4))
            flipped = model(torch.flip(clip, [-1]).permute(0, 2, 1, 3, 4))
            combined = (original + flipped) * .5
        for key, logits in {'single_original': original, 'single_flipped': flipped,
                            'single_hflip': combined}.items():
    ...
    for number, clip_id in enumerate(chosen):
        ...
        for source_idx in timeline:
            try:
                ...
                draw.text((4, 275), 'Yellow: mapped IR crop | right: exact 128px input crop', fill='white')
...
def load_champion_boundary(meta, cfg):
    ...
    if not prob_path.exists() or not meta_path.exists():
        status = dict(status='missing_champion_artifacts', searched=str(root),
                      ...
                      limitation='V9 diagnostics are NOT V8 champion diagnostics. Original/flip champion logits '
...
def train_selection(meta, cfg):
    ...
    fp32_scores, _, _ = diagnostic_report(diagnostic_meta, fp32_probs, cfg,
        'v9_boundary_fp32', f'V9 single-view-selected EMA epoch {best_epoch}; held-out users {cfg.boundary_users}')
    ...
    torch.save(dict(model_int8=packed, classes=LABEL_VALUES.tolist(), split='boundary',
                    ...
                    cache_frames=64, model_frames=32, image_size=128,
    ...
    scores, predictions, folder = diagnostic_report(diagnostic_meta, int8_probs, cfg,
        'v9_boundary_int8', f'Original champion INT8 quantization of V9 boundary epoch {best_epoch}; no full-training subjects')
    ...
    int8_acc = single_acc
    print(f'DIAGNOSTICS V9: FP32 single+hflip={best_acc:.4f}, '
          f'INT8 single+hflip={single_acc:.4f}, selected_test_view=single (fixed)')
    if champion_probs is not None:
        ...
        for precision, probabilities in (('fp32', fp32_probs), ('int8', int8_probs)):
            key = f'v9_{precision}_single'
            comparison[f'{key}_prediction'] = LABEL_VALUES[probabilities['single_hflip'].argmax(1)]
        ...
        comparison.to_csv(cfg.out_dir / 'v8_vs_v9_boundary.csv', index=False)
    (cfg.out_dir / 'test_view_decision.json').write_text(json.dumps(dict(
        criterion='V9 predeclared inference policy; no temporal-view selection and no test labels',
        selected_view='single', horizontal_flip=True,
        single_accuracy=single_acc), indent=2), encoding='utf-8')
    pd.DataFrame([{
        "split": "boundary",
...
def predict_test(model_path, meta, cfg):
    ...
    records = infer_view_logits(model, meta, cfg, 'test')
...
FINAL_MODEL = CFG.out_dir / "thermal_v9_mvitv2s_fullspan_timewarp_int8.pt"
torch.save({
    "model_int8": packed,
    "architecture": "torchvision_mvit_v2_s_relative_position_32x128",
    ...
    "temporal_input": "train: 70% V8 ordered pairs + 30% monotonic endpoint-preserving full-span warp; test: uniform32",
    "fullspan_timewarp_probability": CFG.fullspan_timewarp_p,
    "timewarp_midpoint_range": CFG.timewarp_midpoint_range,
    "temporal_test_view": "single+hflip",
    "temporal_schedule": temporal_indices64('single').tolist(),
...
test_probs = test_views['single_hflip']
np.save(CFG.out_dir / "thermal_test_probs.npy", test_probs)
np.save(CFG.out_dir / "thermal_test_probs_single.npy", test_probs)
...
print(f'Fixed inference: single uniform32 + hflip; outputs retained in {CFG.out_dir}')
