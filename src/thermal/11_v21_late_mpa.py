...
class Config:
    ...
    # Average raw model parameters only in the late, low-LR part of full-data
    # supervised training. This is training-only state and adds zero deployment
    # parameters. Sparse snapshots keep CPU overhead modest.
    mpa_start_update: int = 2244
    mpa_interval: int = 24
    batch_size: int = 2
    ...
    @property
    def out_dir(self):
        return self.work_root / "thermal_v21_mvitv2s_maskfeat_160_late_mpa"
...
assert 0 < CFG.mpa_start_update < CFG.full_train_updates
assert CFG.mpa_interval > 0
...
print('V21: V16-160 recipe plus late model-parameter averaging (MPA); no boundary rerun')
...
print(f'Single hypothesis: average raw checkpoints from update {CFG.mpa_start_update} every '
      f'{CFG.mpa_interval} updates to improve cross-subject basin robustness.')
print('Cache guard: raw Thermal -> fixed IR union crop (margin=1.60) -> direct 160x160 resize; S128 cache is forbidden.')
print('Full-data SSL uses training Thermal only; test Thermal is never used.')
print('V8 fine-tuning remains: 64 cached frames -> ordered 32-bin random sampling; uniform LR; original INT8')
print('No boundary model selection: full-data training length remains fixed at 3740 updates.')
...
@torch.no_grad()
def update_cpu_mpa(running_state, model_state, count):
    """Arithmetic mean of raw model snapshots, held on CPU in float32."""
    new_count = count + 1
    if running_state is None:
        ...
        return running_state, new_count
    old_weight = count / new_count
    new_weight = 1.0 / new_count
    for key, value in model_state.items():
        cpu_value = value.detach().cpu()
        if cpu_value.is_floating_point():
            running_state[key].mul_(old_weight).add_(cpu_value.float(), alpha=new_weight)
        else:
            # Integer counters are not meaningfully averageable; retain latest.
            running_state[key] = cpu_value.clone()
    return running_state, new_count
...
def train_full(meta, target_updates, schedule_total_steps, schedule_warmup_steps, cfg):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for micro_step, (clip, label, _) in enumerate(loader):
            ...
            with torch.autocast("cuda", enabled=scaler.is_enabled()):
                ...
                if not torch.isfinite(raw_loss):
                    raise FloatingPointError(f'non-finite supervised loss at update {step}')
                loss = raw_loss / cfg.grad_accum
            scaler.scale(loss).backward()
            loss_sum += raw_loss.item() * len(label); seen += len(label)
            if (micro_step + 1) % cfg.grad_accum == 0:
                ...
                scaler.step(optimizer); scaler.update(); optimizer.zero_grad(set_to_none=True)
                ema.update(model, step=step); step += 1
                take_mpa = step >= cfg.mpa_start_update and (
                    (step - cfg.mpa_start_update) % cfg.mpa_interval == 0 or
                    step == target_updates)
                if take_mpa:
                    mpa_state, mpa_count = update_cpu_mpa(
                        mpa_state, model.state_dict(), mpa_count)
                    mpa_records.append(dict(update=step, snapshot_count=mpa_count,
        ...
        print(f"full {epoch:02d}/{epochs} updates={step}/{target_updates} "
              ...
              f"time={time.time()-start:.0f}s")
    assert step == target_updates, f'expected {target_updates} updates, got {step}'
    assert mpa_state is not None and mpa_count >= 2, 'late MPA produced too few snapshots'
    assert mpa_records[-1]['update'] == target_updates, 'final raw model was not included in MPA'
    pd.DataFrame(mpa_records).to_csv(cfg.out_dir / 'late_mpa_history.csv', index=False)
    print(f'V21 late MPA: averaged {mpa_count} raw-model snapshots from update '
          f'{mpa_records[0]["update"]} through {mpa_records[-1]["update"]}')
    state = mpa_state
...
def predict_test(model_path, meta, cfg):
    ...
    # V21 official inference remains strictly the V16 single temporal view + hflip.
    if DEVICE.type == 'cuda':
    ...
    print(f"V21 test inference: {inference_seconds:.2f}s total, "
...
print('V21 intentionally skips boundary MaskFeat and boundary supervised training.')
...
FINAL_MODEL = CFG.out_dir / "thermal_v21_mvitv2s_maskfeat_160_late_mpa_int8.pt"
torch.save({
    "model_int8": packed,
    "architecture": "torchvision_mvit_v2_s_relative_position_32x160",
    ...
    "experiment_hypothesis": "late arithmetic model-parameter averaging improves cross-subject basin robustness",
    ...
    "parameter_averaging": "arithmetic mean of raw model state snapshots; non-floating buffers retain latest",
    "mpa_start_update": CFG.mpa_start_update,
    "mpa_interval": CFG.mpa_interval,
...
print('V21 deploys one MPA-averaged INT8 model; averaging adds no inference parameters or forwards.')

test_views = predict_test(FINAL_MODEL, test_meta, CFG)
test_probs = test_views[f'{selected_view}_hflip']
assert test_probs.shape == (len(test_meta), N_CLASSES)
assert np.isfinite(test_probs).all() and np.allclose(test_probs.sum(1), 1.0, atol=1e-4)
