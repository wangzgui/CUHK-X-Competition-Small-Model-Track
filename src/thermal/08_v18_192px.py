...
class Config:
    ...
    @property
    def out_dir(self):
        return self.work_root / "thermal_v18_mvitv2s_maskfeat_192_v16_finetune"
...
assert CFG.batch_size >= 2, "V18 requires physical batch >=2 so V16 Video MixUp remains active"
...
print('V18: V16 recipe at native 192x192 spatial sampling; no V17 high-grid topology and no spatial-zoom TTA')
...
print('Single hypothesis: raise end-to-end native spatial sampling while preserving each HOG target at 2x32x32 pixels.')
...
def build_cache(clips, windows, split, cfg):
    ...
    signature_data = dict(version='v18_raw_thermal_direct192_uniform64', shape=shape,
...
if stale_test_frame_cache.exists():
    ...
    print(f'V18 disk guard: removed reproducible stale test frame cache ({released_gib:.2f} GiB) before training.')
...
print('V18 disk guard: defer the 192x192 test cache until training finishes; train and test frame caches are never resident together.')
...
def predict_test(model_path, meta, cfg):
    ...
    # V18 official inference is strictly the V16 single temporal view + hflip.
    if DEVICE.type == 'cuda':
    ...
    print(f"V18 test inference: {inference_seconds:.2f}s total, "
...
FINAL_MODEL = CFG.out_dir / "thermal_v18_mvitv2s_maskfeat_192_v16_finetune_int8.pt"
torch.save({
    "model_int8": packed,
    "architecture": "torchvision_mvit_v2_s_relative_position_32x192",
    ...
    "experiment_hypothesis": "end-to-end native 192 sampling with unchanged 2x32x32-pixel HOG target physics",
...
if train_frame_cache.exists():
    ...
    print(f'V18 disk guard: removed reproducible train frame cache ({released_gib:.2f} GiB) before test caching.')
gc.collect()
