...
class Config:
    ...
    # Set this explicitly only if the uploaded Kaggle dataset renamed the file.
    # By default V23 searches every mounted /kaggle/input dataset for the exact
    # V16 full-training deployment artifact and rejects boundary-only weights.
    v16_checkpoint: Path | None = None
    v16_checkpoint_filename: str = "thermal_v16_mvitv2s_maskfeat_160_v11_finetune_int8.pt"
    # Required external training metadata, not an inference dependency. The NPZ
    # must contain probs, clip_ids, action_ids, class_ids, users, fold_ids and is_oof.
    visual_oof_path: Path | None = None
    visual_oof_filename: str = "visual_cross_subject_oof.npz"
    ...
    # Short V16-preserving visual-error-aware adaptation.
    adaptation_updates: int = 935
    adaptation_lr: float = 1e-5
    adaptation_min_lr: float = 1e-6
    adaptation_warmup_steps: int = 47
    visual_reweight_strength: float = 0.50
    ...
    @property
    def cache_dir(self):
        return self.cache_root / f"V23_Tcache{self.cache_frames}_S{self.img_size}_IRcrop_uniform_v1"

    @property
    def out_dir(self):
        return self.work_root / "thermal_v23_mvitv2s_160_visual_oof_reweight"


def locate_v16_checkpoint(cfg):
    if cfg.v16_checkpoint is not None:
        candidates = [Path(cfg.v16_checkpoint)]
    else:
        candidates = [cfg.work_root / cfg.v16_checkpoint_filename]
        input_root = Path('/kaggle/input')
        if input_root.exists():
            candidates.extend(input_root.rglob(cfg.v16_checkpoint_filename))
    candidates = [path.resolve() for path in candidates if path.is_file()]
    if not candidates:
        raise FileNotFoundError(
            'V23 requires the FULL-TRAINING V16 model before any cache is built. '
            f'Upload and attach {cfg.v16_checkpoint_filename} as a Kaggle dataset, '
            'or set Config.v16_checkpoint to its exact mounted path. '
            'Do not use boundary_best_int8_DIAGNOSTIC_ONLY.pt.')
    selected = sorted(set(candidates), key=str)[0]
    if 'DIAGNOSTIC_ONLY' in selected.name or 'boundary' in selected.name.lower():
        raise ValueError(f'Boundary-only checkpoint is forbidden for V23: {selected}')
    return selected


def locate_visual_oof(cfg):
    if cfg.visual_oof_path is not None:
        candidates = [Path(cfg.visual_oof_path)]
    else:
        candidates = [cfg.work_root / cfg.visual_oof_filename]
        input_root = Path('/kaggle/input')
        if input_root.exists():
            candidates.extend(input_root.rglob(cfg.visual_oof_filename))
    candidates = [path.resolve() for path in candidates if path.is_file()]
    if not candidates:
        raise FileNotFoundError(
            'V23 requires complete cross-subject visual OOF probabilities before any cache is built. '
            f'Upload {cfg.visual_oof_filename}, or set Config.visual_oof_path. Required NPZ keys: '
            'probs [N,40], clip_ids [N], action_ids [N], class_ids [40], users [N], '
            'fold_ids [N], is_oof [N]. '
            'The existing 541/545-row boundary files are forbidden.')
...
assert (CFG.adaptation_updates, CFG.adaptation_lr, CFG.adaptation_warmup_steps) == (935, 1e-5, 47)
assert CFG.visual_reweight_strength == 0.50
...
assert "S160" in CFG.cache_dir.name and "S192" not in CFG.cache_dir.name and "S128" not in CFG.cache_dir.name
...
assert CFG.batch_size >= 2, "V23 requires physical batch >=2 so V16 Video MixUp remains active"
assert CFG.tta_hflip and CFG.dual_min_extra_correct >= 1
V16_CHECKPOINT = locate_v16_checkpoint(CFG)
VISUAL_OOF_PATH = locate_visual_oof(CFG)
...
print('V16 full-training initialization:', V16_CHECKPOINT)
print('Cross-subject visual OOF:', VISUAL_OOF_PATH)
...
print('V23: V16-160 plus class-conditional visual-OOF difficulty reweighting')
print(f'Adaptation: updates={CFG.adaptation_updates}, lr={CFG.adaptation_lr:.1e}, '
      f'min_lr={CFG.adaptation_min_lr:.1e}, warmup={CFG.adaptation_warmup_steps}')
print(f'Visual difficulty strength={CFG.visual_reweight_strength:.2f}; weights are globally mean-normalized.')
print('Single hypothesis: emphasize samples that an honest visual OOF model finds difficult without matching its logits.')
print('Cache guard: raw Thermal -> fixed IR union crop (margin=1.60) -> direct 160x160 resize; S128/S192 caches are forbidden.')
print('No boundary training, no MaskFeat rerun, no classifier reinitialization, and no visual distillation.')
print('V16 fine-tuning recipe remains: ordered 32-bin random sampling, MixUp, calibration, EMA, original INT8.')
...
def load_visual_oof_weights(path, clips, cfg):
    required = {'probs', 'clip_ids', 'action_ids', 'class_ids', 'users', 'fold_ids', 'is_oof'}
    with np.load(path, allow_pickle=False) as artifact:
        missing = required.difference(artifact.files)
        if missing:
            raise KeyError(f'Visual OOF artifact missing keys: {sorted(missing)}')
        probs = np.asarray(artifact['probs'], dtype=np.float32)
        clip_ids = np.asarray(artifact['clip_ids']).astype(str)
        action_ids = np.asarray(artifact['action_ids'], dtype=np.int64)
        class_ids = np.asarray(artifact['class_ids'], dtype=np.int64)
        users = np.asarray(artifact['users']).astype(str)
        fold_ids = np.asarray(artifact['fold_ids'], dtype=np.int64)
        is_oof = np.asarray(artifact['is_oof']).astype(bool)

    n_required = len(clips)
    n_oof = len(clip_ids)
    assert probs.shape == (n_oof, N_CLASSES), f'Expected OOF probabilities shaped (N,{N_CLASSES}), got {probs.shape}'
    assert all(len(x) == n_oof for x in (action_ids, users, fold_ids, is_oof))
    assert np.array_equal(class_ids, LABEL_VALUES), 'Visual OOF class_ids/order mismatch'
    assert len(np.unique(clip_ids)) == n_oof, 'Visual OOF contains duplicate clip_ids'
    assert is_oof.all(), 'Every visual prediction must be explicitly marked out-of-fold'
    assert (fold_ids >= 0).all() and len(np.unique(fold_ids)) >= 2, 'Invalid OOF fold IDs'
    if not np.isfinite(probs).all() or (probs < 0).any():
        raise FloatingPointError('Visual OOF contains invalid probabilities')
    if not np.allclose(probs.sum(1), 1.0, atol=1e-4):
        raise ValueError('Visual OOF probability rows do not sum to one')

    expected = pd.DataFrame({
        'clip_id': [c.clip_id for c in clips],
        'action_id_expected': [c.action_id for c in clips],
        'user_expected': [c.user for c in clips],
    })
    supplied = pd.DataFrame({
        'clip_id': clip_ids, 'action_id': action_ids, 'user': users,
        'fold_id': fold_ids, 'artifact_row': np.arange(n_oof),
    })
    aligned = expected.merge(supplied, on='clip_id', how='left', validate='one_to_one', sort=False)
    assert aligned.artifact_row.notna().all(), 'Visual OOF does not cover every Thermal training clip'
    aligned['artifact_row'] = aligned.artifact_row.astype(int)
    assert np.array_equal(aligned.action_id.to_numpy(), aligned.action_id_expected.to_numpy())
    assert np.array_equal(aligned.user.to_numpy(), aligned.user_expected.to_numpy())
    # Cross-subject folds require all clips from one subject to share one held-out fold.
    user_fold_counts = aligned.groupby('user', sort=False).fold_id.nunique()
    assert (user_fold_counts == 1).all(), 'A subject appears in multiple visual OOF folds'

    order = aligned.artifact_row.to_numpy()
    aligned_probs = probs[order]
    true_columns = np.asarray([ID_TO_INDEX[int(x)] for x in aligned.action_id], dtype=int)
    true_probability = aligned_probs[np.arange(n_required), true_columns]
    difficulty = 1.0 - true_probability
    difficulty_table = pd.DataFrame({'action_id': aligned.action_id.to_numpy(),
                                     'difficulty': difficulty})
    within_class_percentile = difficulty_table.groupby('action_id', sort=False).difficulty.rank(
        method='average', pct=True).to_numpy(dtype=np.float32)
    raw_weight = 1.0 + cfg.visual_reweight_strength * within_class_percentile
    sample_weight = (raw_weight / raw_weight.mean()).astype(np.float32)
    assert np.isfinite(sample_weight).all() and (sample_weight > 0).all()
    assert abs(float(sample_weight.mean()) - 1.0) < 1e-6
    assert float(sample_weight.max() / sample_weight.min()) <= 1.0 + cfg.visual_reweight_strength + 1e-5

    aligned['visual_true_probability'] = true_probability
    aligned['visual_prediction'] = LABEL_VALUES[aligned_probs.argmax(1)]
    aligned['visual_correct'] = aligned.visual_prediction.to_numpy() == aligned.action_id.to_numpy()
    aligned['visual_difficulty'] = difficulty
    aligned['within_class_difficulty_percentile'] = within_class_percentile
    aligned['thermal_training_weight'] = sample_weight
    aligned.drop(columns=['artifact_row']).to_parquet(
        cfg.out_dir / 'visual_oof_weight_audit.parquet', index=False)
    print(f'Visual OOF QA: source_rows={n_oof}, aligned_thermal_rows={n_required}, '
          f'users={aligned.user.nunique()}, folds={aligned.fold_id.nunique()}, '
          f'accuracy={aligned.visual_correct.mean():.4f}, weight_range='
          f'[{sample_weight.min():.3f}, {sample_weight.max():.3f}], mean={sample_weight.mean():.3f}')
    return aligned[['clip_id']].assign(sample_weight=sample_weight)


VISUAL_WEIGHT_TABLE = load_visual_oof_weights(VISUAL_OOF_PATH, train_clips, CFG)
...
def build_cache(clips, windows, split, cfg):
    ...
    signature_data = dict(version='v23_raw_thermal_direct160_uniform64', shape=shape,
...
# On a rerun after completed inference, reclaim any reproducible test frame
# array before checking space for the much larger training array.
stale_test_frame_cache = (CFG.cache_dir / 'test_thermal.npy').resolve()
assert stale_test_frame_cache.parent == CFG.cache_dir.resolve()
if stale_test_frame_cache.exists():
    released_gib = stale_test_frame_cache.stat().st_size / 1024**3
    stale_test_frame_cache.unlink()
    print(f'V23 disk guard: removed reproducible stale test frame cache ({released_gib:.2f} GiB) before training.')
...
weight_alignment = train_meta[['clip_id']].merge(
    VISUAL_WEIGHT_TABLE, on='clip_id', how='left', validate='one_to_one', sort=False)
assert weight_alignment.sample_weight.notna().all()
TRAIN_SAMPLE_WEIGHTS = weight_alignment.sample_weight.to_numpy(dtype=np.float32)
assert len(TRAIN_SAMPLE_WEIGHTS) == len(train_meta) and abs(float(TRAIN_SAMPLE_WEIGHTS.mean()) - 1.0) < 1e-6
print('V23 disk guard: defer the 160x160 test cache until training finishes; train and test frame caches are never resident together.')
...
def weighted_video_mixup(clip, label, sample_weight, cfg):
    """V16 MixUp with the same convex combination applied to OOF weights."""
    assert len(clip) == len(label) == len(sample_weight)
    targets = F.one_hot(label, N_CLASSES).float()
    targets = targets * (1.0 - cfg.label_smoothing) + cfg.label_smoothing / N_CLASSES
    mixed = False
    lam = 1.0
    if len(clip) > 1 and random.random() < cfg.mixup_p:
        ...
        clip = clip * lam + clip[permutation] * (1.0 - lam)
        targets = targets * lam + targets[permutation] * (1.0 - lam)
        sample_weight = sample_weight * lam + sample_weight[permutation] * (1.0 - lam)
        mixed = True
    return clip, targets, sample_weight, mixed, lam
...
def weighted_soft_target_cross_entropy(logits, targets, sample_weight):
    per_sample = -(targets * F.log_softmax(logits, dim=1)).sum(dim=1)
    assert per_sample.shape == sample_weight.shape
    # Weights are globally mean-normalized. Do not renormalize within a
    # physical batch of two, which would erase between-batch difficulty scale.
    return (per_sample * sample_weight).mean(), per_sample.mean()
...
def train_selection(meta, cfg):
    ...
    fp32_scores, _, _ = diagnostic_report(diagnostic_meta, fp32_probs, cfg,
        'v23_boundary_fp32', f'V23 native-160 EMA epoch {best_epoch}; held-out users {cfg.boundary_users}')
    ...
    torch.save(dict(model_int8=packed, classes=LABEL_VALUES.tolist(), split='boundary',
                    ...
                    cache_frames=64, model_frames=32, image_size=cfg.img_size,
    ...
    scores, predictions, folder = diagnostic_report(diagnostic_meta, int8_probs, cfg,
        'v23_boundary_int8', f'V23 native-160 INT8 boundary epoch {best_epoch}; no full-training subjects')
    ...
    int8_acc = single_acc
    print(f'DIAGNOSTICS V23: FP32 single+hflip={best_acc:.4f}, INT8 single+hflip={single_acc:.4f}, '
    ...
    if champion_probs is not None:
        ...
        comparison.to_csv(cfg.out_dir / 'legacy_champion_vs_v23_boundary.csv', index=False)
    (cfg.out_dir / 'test_view_decision.json').write_text(json.dumps(dict(
        criterion='V23 diagnostics only; deployment remains fixed to V16 single+hflip',
...
def load_v16_model(cfg):
    checkpoint = torch.load(V16_CHECKPOINT, map_location='cpu', weights_only=False)
    if not isinstance(checkpoint, dict):
        raise TypeError('V16 checkpoint must be a dictionary artifact')
    if checkpoint.get('split') == 'boundary':
        raise ValueError('V23 refuses boundary-only V16 weights')
    if 'classes' in checkpoint:
        loaded_classes = np.asarray(checkpoint['classes'], dtype=int)
        assert np.array_equal(loaded_classes, LABEL_VALUES), 'V16 checkpoint class order mismatch'
    if 'image_size' in checkpoint:
        assert int(checkpoint['image_size']) == 160, checkpoint['image_size']
    if 'full_train_updates' in checkpoint:
        assert int(checkpoint['full_train_updates']) == 3740, checkpoint['full_train_updates']
    if 'model_int8' in checkpoint:
        state = dequantize_state_dict(checkpoint['model_int8'])
        source_precision = 'INT8-dequantized'
    else:
        state = checkpoint.get('state_dict', checkpoint.get('model'))
        if state is None:
            raise KeyError('V16 checkpoint has neither model_int8, state_dict, nor model weights')
        source_precision = 'FP32'
    model = build_model(False, cfg)
    model.load_state_dict(state, strict=True)
    del state, checkpoint
    print(f'V23 checkpoint QA: full-training V16, source={source_precision}, strict_state_load=True')
    return model


def adaptation_lr_at(step, cfg):
    if step < cfg.adaptation_warmup_steps:
        return cfg.adaptation_lr * (step + 1) / max(cfg.adaptation_warmup_steps, 1)
    progress = (step - cfg.adaptation_warmup_steps) / max(
        cfg.adaptation_updates - cfg.adaptation_warmup_steps, 1)
    progress = min(max(progress, 0.0), 1.0)
    return cfg.adaptation_min_lr + .5 * (cfg.adaptation_lr - cfg.adaptation_min_lr) * (
        1 + math.cos(math.pi * progress))


def train_visual_oof_reweight(meta, sample_weights, cfg):
    seed_everything(cfg.seed + 200)
    npy = cfg.cache_dir / 'train_thermal.npy'
    model = load_v16_model(cfg).to(DEVICE)
    loader = DataLoader(ThermalDataset(npy, meta, cfg, True), cfg.batch_size, shuffle=True,
                        drop_last=True, **common_loader_args(cfg))
    ema = ModelEmaV3(model, decay=cfg.ema_decay, device=DEVICE)
    optimizer = torch.optim.AdamW(model.parameters(), lr=cfg.adaptation_lr,
                                  weight_decay=cfg.weight_decay)
    scaler = torch.amp.GradScaler('cuda', enabled=cfg.amp and DEVICE.type == 'cuda')
    updates_per_epoch = len(loader) // cfg.grad_accum
    assert updates_per_epoch > 0
    epochs = math.ceil(cfg.adaptation_updates / updates_per_epoch)
    step, history = 0, []
    for epoch in range(1, epochs + 1):
        model.train(); optimizer.zero_grad(set_to_none=True)
        weighted_sum = unweighted_sum = weight_sum = 0.0
        seen = mixed_microbatches = microbatches = 0; start = time.time()
        remaining_updates = cfg.adaptation_updates - step
        usable_microbatches = min(updates_per_epoch, remaining_updates) * cfg.grad_accum
        for micro_step, (clip, label, idx) in enumerate(loader):
            if micro_step >= usable_microbatches or step >= cfg.adaptation_updates:
                break
            clip, label = clip.to(DEVICE, non_blocking=True), label.to(DEVICE, non_blocking=True)
            batch_weight = torch.from_numpy(sample_weights[idx.numpy()]).to(
                DEVICE, non_blocking=True)
            clip, soft_targets, batch_weight, mixed, _ = weighted_video_mixup(
                clip, label, batch_weight, cfg)
            with torch.autocast('cuda', enabled=scaler.is_enabled()):
                logits = model(clip.permute(0, 2, 1, 3, 4))
                raw_loss, unweighted_ce = weighted_soft_target_cross_entropy(
                    logits, soft_targets, batch_weight)
                loss = raw_loss / cfg.grad_accum
            if not torch.isfinite(raw_loss) or not torch.isfinite(unweighted_ce):
                raise FloatingPointError(f'Non-finite V23 loss at update {step}')
            if epoch == 1 and micro_step == 0:
                print(f'V23 weight QA: batch={batch_weight.detach().cpu().tolist()}, '
                      f'weighted_ce={raw_loss.item():.5f}, unweighted_ce={unweighted_ce.item():.5f}')
            scaler.scale(loss).backward()
            weighted_sum += raw_loss.item() * len(label)
            unweighted_sum += unweighted_ce.item() * len(label)
            weight_sum += batch_weight.detach().sum().item()
            seen += len(label); microbatches += 1; mixed_microbatches += int(mixed)
            if (micro_step + 1) % cfg.grad_accum == 0:
                for group in optimizer.param_groups:
                    group['lr'] = adaptation_lr_at(step, cfg)
                scaler.unscale_(optimizer)
                nn.utils.clip_grad_norm_(model.parameters(), cfg.grad_clip)
                scaler.step(optimizer); scaler.update(); optimizer.zero_grad(set_to_none=True)
                ema.update(model, step=step); step += 1
        history.append(dict(epoch=epoch, optimizer_updates=step,
                            lr=optimizer.param_groups[0]['lr'],
                            weighted_train_loss=weighted_sum / max(seen, 1),
                            unweighted_train_loss=unweighted_sum / max(seen, 1),
                            effective_weight_mean=weight_sum / max(seen, 1),
                            mixup_microbatch_fraction=mixed_microbatches / max(microbatches, 1),
                            seconds=time.time()-start))
        pd.DataFrame(history).to_csv(cfg.out_dir / 'visual_oof_reweight_history.csv', index=False)
        print(f'V23 adaptation {epoch:02d}/{epochs} updates={step}/{cfg.adaptation_updates} '
              f'lr={optimizer.param_groups[0]["lr"]:.2e} '
              f'weighted={weighted_sum/max(seen,1):.4f} '
              f'unweighted={unweighted_sum/max(seen,1):.4f} time={time.time()-start:.0f}s')
    assert step == cfg.adaptation_updates, (step, cfg.adaptation_updates)
    state = {key: value.detach().cpu().clone() for key, value in ema.module.state_dict().items()}
...
def predict_test(model_path, meta, cfg):
    ...
    # V23 official inference is strictly the V16 single temporal view + hflip.
    if DEVICE.type == 'cuda':
    ...
    print(f"V23 test inference: {inference_seconds:.2f}s total, "
...
selected_view = 'single'
print('V23 begins short adaptation directly: boundary selection and duplicate MaskFeat are skipped.')
final_state = train_visual_oof_reweight(train_meta, TRAIN_SAMPLE_WEIGHTS, CFG)
packed = quantize_state_dict(final_state)
FINAL_MODEL = CFG.out_dir / "thermal_v23_mvitv2s_160_visual_oof_reweight_int8.pt"
torch.save({
    "model_int8": packed,
    "architecture": "torchvision_mvit_v2_s_relative_position_32x160",
    ...
    "initialization": "completed full-training V16-160 deployment checkpoint",
    "initialization_path": str(V16_CHECKPOINT),
    "pretraining": "inherited V16 Kinetics plus CUHK Thermal MaskFeat representation; not rerun in V23",
    "experiment_hypothesis": "increase Thermal attention to samples difficult for honest cross-subject visual OOF",
    ...
    "adaptation_updates": CFG.adaptation_updates,
    "adaptation_lr": CFG.adaptation_lr,
    "adaptation_min_lr": CFG.adaptation_min_lr,
    "adaptation_warmup_steps": CFG.adaptation_warmup_steps,
    "visual_oof_artifact": str(VISUAL_OOF_PATH),
    "visual_reweight_strength": CFG.visual_reweight_strength,
    "visual_reweight_formula": "1 + strength * within-true-class percentile(1 - visual OOF p_true), global mean normalization",
    "visual_distillation": False,
    "boundary_training_skipped": True,
    "maskfeat_rerun": False,
...
print('V23 saved its final deployment model before deleting the reproducible training frame cache.')

# Keep only one large reproducible frame array resident at a time.
# Training is complete and the final INT8 artifact is already durable, so remove
# only the reproducible train frame array before creating the test frame array.
# Metadata and diagnostics are retained; rerunning the notebook can regenerate
# this exact cache from the raw Thermal images and recorded preprocessing.
train_frame_cache = (CFG.cache_dir / 'train_thermal.npy').resolve()
assert train_frame_cache.parent == CFG.cache_dir.resolve()
if train_frame_cache.exists():
    released_gib = train_frame_cache.stat().st_size / 1024**3
    train_frame_cache.unlink()
    print(f'V23 disk guard: removed reproducible train frame cache ({released_gib:.2f} GiB) before test caching.')
gc.collect()
...
diagnostics_dir = CFG.out_dir / 'diagnostics'
if diagnostics_dir.exists():
    shutil.make_archive(str(CFG.out_dir / 'diagnostics_bundle'), 'zip', diagnostics_dir)
