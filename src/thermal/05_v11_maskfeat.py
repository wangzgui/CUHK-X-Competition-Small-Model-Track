...
class Config:
    ...
    @property
    def out_dir(self):
        return self.work_root / "thermal_v11_mvitv2s_maskfeat_pretrain_v8_finetune"
...
print('V11: Thermal-only MaskFeat-style HOG pre-training -> unchanged V8 supervised fine-tuning')
...
def thermal_hog_targets(normalized_clip, cfg):
    """Return L2-normalized 9-bin unsigned HOG on the final 16x4x4 token grid."""
...
def train_selection(meta, cfg):
    ...
    (cfg.out_dir / 'test_view_decision.json').write_text(json.dumps(dict(
        criterion='V11 isolates MaskFeat; deployment is fixed to the V8 single+hflip view',
...
FINAL_MODEL = CFG.out_dir / "thermal_v11_mvitv2s_maskfeat_pretrain_v8_finetune_int8.pt"
torch.save({
    "model_int8": packed,
    "architecture": "torchvision_mvit_v2_s_relative_position_32x128",
