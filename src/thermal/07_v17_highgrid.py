...
class Config:
    ...
    # V17 changes only the SSL topology: block 14 keeps its 10x10 spatial
    # resolution during MaskFeat. Supervised fine-tuning and inference rebuild
    # the untouched Torchvision MViTv2-S and therefore retain the V16 5x5 grid.
    maskfeat_final_grid: tuple[int, int, int] = (16, 10, 10)
    supervised_final_grid: tuple[int, int, int] = (16, 5, 5)
    maskfeat_high_grid_block: int = 14
    ...
    @property
    def out_dir(self):
        return self.work_root / "thermal_v17_mvitv2s_maskfeat_highgrid_160_v16_finetune"


CFG = Config()
assert (CFG.cache_frames, CFG.model_frames, CFG.img_size, CFG.batch_size, CFG.grad_accum) == (64, 32, 160, 2, 6)
assert CFG.maskfeat_final_grid == (16, 10, 10) and CFG.maskfeat_hog_bins == 9
assert CFG.supervised_final_grid == (16, 5, 5)
assert CFG.model_frames // CFG.maskfeat_final_grid[0] == 2
assert CFG.img_size // CFG.maskfeat_final_grid[1] == 16
...
assert CFG.batch_size >= 2, "V17 requires physical batch >=2 so V16 Video MixUp remains active"
...
print('V17: V16-160 baseline plus high-grid MaskFeat pre-training only; no spatial-zoom TTA')
...
print('Single hypothesis: preserve the block-14 10x10 SSL grid so each HOG target covers about 2x16x16 pixels.')
print(f'Supervised/inference backbone is rebuilt unchanged at final grid={CFG.supervised_final_grid}.')
...
def configure_high_grid_maskfeat_backbone(backbone, cfg):
    """Keep block 14 at 10x10 for SSL without adding any learned parameters.

    Torchvision MViTv2-S normally downsamples 10x10 -> 5x5 in block 14.
    MaskFeat's official MViTv2-S recipe instead uses spatial stride one in the
    final pooling block during pre-training. The query depthwise convolution
    keeps its pretrained weights and 3x3 kernel; only its stride changes. The
    stride-two residual MaxPool is removed so the residual and attention paths
    have the same token count. A fresh standard backbone is built later for
    supervised fine-tuning, so this training-only topology cannot leak into the
    submitted model.
    """
    block_index = cfg.maskfeat_high_grid_block
    assert block_index == 14 and len(backbone.blocks) == 16
    block = backbone.blocks[block_index]
    assert block.attn.pool_q is not None and isinstance(block.attn.pool_q.pool, nn.Conv3d)
    assert tuple(block.attn.pool_q.pool.kernel_size) == (3, 3, 3)
    assert tuple(block.attn.pool_q.pool.stride) == (1, 2, 2)
    assert block.pool_skip is not None and isinstance(block.pool_skip.pool, nn.MaxPool3d)
    assert tuple(block.pool_skip.pool.stride) == (1, 2, 2)
    block.attn.pool_q.pool.stride = (1, 1, 1)
    block.pool_skip = None
    assert tuple(block.attn.pool_q.pool.stride) == (1, 1, 1) and block.pool_skip is None
    return backbone


class ThermalMaskFeat(nn.Module):
    """Mask high-grid patch regions and predict one local 9-bin HOG per token."""
    def __init__(self, backbone, cfg):
        super().__init__()
        self.backbone = configure_high_grid_maskfeat_backbone(backbone, cfg)
        self.cfg = cfg
...
def thermal_hog_targets(normalized_clip, cfg):
    """Return L2-normalized 9-bin unsigned HOG on the SSL 16x10x10 grid."""
...
standard_state_schema = {k: tuple(v.shape) for k, v in probe_model.state_dict().items()}
del probe_output
del probe_model

# V17 structural guard: prove the training-only model emits 16x10x10 HOG
# predictions and has exactly the same learned state schema as standard V16.
probe_ssl = ThermalMaskFeat(build_model(False, CFG), CFG).eval()
ssl_state_schema = {k: tuple(v.shape) for k, v in probe_ssl.backbone.state_dict().items()}
assert ssl_state_schema == standard_state_schema, "High-grid SSL changed learned parameter keys or shapes"
probe_mask = final_grid_mask(1, CFG, torch.device('cpu'))
with torch.no_grad():
    probe_ssl_output = probe_ssl(
        torch.zeros(1, 3, CFG.model_frames, CFG.img_size, CFG.img_size), probe_mask)
assert tuple(probe_ssl_output.shape) == (1, int(np.prod(CFG.maskfeat_final_grid)), CFG.maskfeat_hog_bins)
print(f"V17 high-grid MaskFeat preflight: prediction={tuple(probe_ssl_output.shape)}, "
      "block14 q_stride=(1,1,1), learned state schema unchanged")
del probe_ssl_output, probe_mask, probe_ssl
gc.collect()
...
def build_v8_finetune_model(maskfeat_state, cfg, finetune_seed):
    """Load the high-grid SSL encoder into the untouched V16-160 classifier."""
...
def train_selection(meta, cfg):
    ...
    fp32_scores, _, _ = diagnostic_report(diagnostic_meta, fp32_probs, cfg,
        'v17_boundary_fp32', f'V17 high-grid MaskFeat EMA epoch {best_epoch}; held-out users {cfg.boundary_users}')
    ...
    scores, predictions, folder = diagnostic_report(diagnostic_meta, int8_probs, cfg,
        'v17_boundary_int8', f'V17 high-grid MaskFeat INT8 boundary epoch {best_epoch}; no full-training subjects')
    ...
    int8_acc = single_acc
    print(f'DIAGNOSTICS V17: FP32 single+hflip={best_acc:.4f}, INT8 single+hflip={single_acc:.4f}, '
    ...
    if champion_probs is not None:
        ...
        comparison.to_csv(cfg.out_dir / 'legacy_champion_vs_v17_boundary.csv', index=False)
    (cfg.out_dir / 'test_view_decision.json').write_text(json.dumps(dict(
        criterion='V17 changes only MaskFeat block-14 spatial stride; deployment remains V16 single+hflip',
...
def predict_test(model_path, meta, cfg):
    ...
    # V17 official inference is strictly the V16 single temporal view + hflip.
    if DEVICE.type == 'cuda':
    ...
    print(f"V17 test inference: {inference_seconds:.2f}s total, "
...
FINAL_MODEL = CFG.out_dir / "thermal_v17_mvitv2s_maskfeat_highgrid_160_v16_finetune_int8.pt"
torch.save({
    "model_int8": packed,
    "architecture": "torchvision_mvit_v2_s_relative_position_32x160",
    ...
    "pretraining": "Thermal-only high-grid MaskFeat; block 14 spatial query stride changed 2 to 1 during SSL only",
    "experiment_hypothesis": "finer real SSL feature tokens improve hand-object layout without changing deployment",
    ...
    "supervised_final_grid": CFG.supervised_final_grid,
    "maskfeat_high_grid_block": CFG.maskfeat_high_grid_block,
    "maskfeat_high_grid_training_only": True,
