...
class Config:
    ...
    @property
    def out_dir(self):
        return self.work_root / "thermal_v8_mvitv2s_cache64_sample32"
...
print('V8: 64 cached frames -> ordered 32-bin random sampling; uniform LR; original INT8')
print('Checkpoint selection: single uniform32 + hflip. Same-checkpoint temporal-view comparison follows.')
print('Test exports BOTH single and dual views: 6 forwards total/sample, sequential at physical batch 2.')
...
def infer_view_logits(model, meta, cfg, split):
    ...
    records = {f'{view}_{kind}': np.zeros((len(meta), N_CLASSES), np.float32)
               for view in ('single', 'phase_a', 'phase_b', 'dual')
               for kind in ('original', 'flipped', 'hflip')}
    for views, _, idx in tqdm(loader, desc=f'{split}: single/dual + flip diagnostics'):
        batch_logits = {}
        for v, view in enumerate(('single', 'phase_a', 'phase_b')):
            clip = views[:, v].to(DEVICE, non_blocking=True)
            with torch.autocast('cuda', enabled=cfg.amp and DEVICE.type == 'cuda'):
...
def train_selection(meta, cfg):
    ...
    selected_view = 'dual' if extra >= cfg.dual_min_extra_correct else 'single'
    int8_acc = dual_acc if selected_view == 'dual' else single_acc
    ...
    (cfg.out_dir / 'test_view_decision.json').write_text(json.dumps(dict(
        criterion='same single-selected checkpoint; INT8 held-out accuracy; no test labels',
...
FINAL_MODEL = CFG.out_dir / "thermal_v8_mvitv2s_cache64_sample32_int8.pt"
torch.save({
    "model_int8": packed,
    "architecture": "torchvision_mvit_v2_s_relative_position_32x128",
...
for view in ('single', 'dual'):
    np.save(CFG.out_dir / f'thermal_test_probs_{view}.npy', test_views[f'{view}_hflip'])
    variant = test_csv[['path']].copy()
    variant['prediction'] = LABEL_VALUES[test_views[f'{view}_hflip'].argmax(1)].astype(int)
    variant.to_csv(CFG.out_dir / f'thermal_submission_{view}.csv', index=False)
...
print(f'Selected main submission: {selected_view}+hflip; both alternatives retained in {CFG.out_dir}')
