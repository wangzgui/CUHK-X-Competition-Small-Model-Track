...
class Config:
    ...
    # By default V22 searches every mounted /kaggle/input dataset for the exact
    ...
    # Short V16-preserving temporal-consistency adaptation. There is no
    # confidence gate: every microbatch receives the ramped latent loss.
    adaptation_updates: int = 935
    adaptation_lr: float = 1e-5
    adaptation_min_lr: float = 1e-6
    adaptation_warmup_steps: int = 47
    consistency_weight: float = 0.10
    consistency_ramp_updates: int = 187
    ...
    @property
    def cache_dir(self):
        return self.cache_root / f"V22_Tcache{self.cache_frames}_S{self.img_size}_IRcrop_uniform_v1"

    @property
    def out_dir(self):
        return self.work_root / "thermal_v22_mvitv2s_160_temporal_latent_consistency"
...
def locate_v16_checkpoint(cfg):
    ...
    if not candidates:
        raise FileNotFoundError(
            'V22 requires the FULL-TRAINING V16 model before any cache is built. '
    ...
    if 'DIAGNOSTIC_ONLY' in selected.name or 'boundary' in selected.name.lower():
        raise ValueError(f'Boundary-only checkpoint is forbidden for V22: {selected}')
    return selected
...
assert (CFG.consistency_weight, CFG.consistency_ramp_updates) == (0.10, 187)
...
assert CFG.batch_size >= 2, "V22 requires physical batch >=2 so V16 Video MixUp remains active"
...
print('V22: V16-160 plus dual-sampling EMA latent consistency; no confidence gate')
...
print(f'Latent consistency: max_weight={CFG.consistency_weight:.2f}, '
      f'linear_ramp={CFG.consistency_ramp_updates} updates, applied to every microbatch')
print('Single hypothesis: make high-level Thermal action features invariant to legal V8 temporal sampling.')
print('Cache guard: raw Thermal -> fixed IR union crop (margin=1.60) -> direct 160x160 resize; S128/S192 caches are forbidden.')
print('No boundary training, no MaskFeat rerun, no classifier reinitialization, no external data, and no gate.')
print('Both temporal views share spatial/calibration augmentation and MixUp parameters; only sampled frames differ.')
...
def build_cache(clips, windows, split, cfg):
    ...
    signature_data = dict(version='v22_raw_thermal_direct160_uniform64', shape=shape,
...
if stale_test_frame_cache.exists():
    ...
    print(f'V22 disk guard: removed reproducible stale test frame cache ({released_gib:.2f} GiB) before training.')
...
print('V22 disk guard: defer the 160x160 test cache until training finishes; train and test frame caches are never resident together.')
...
class TemporalPairDataset(ThermalDataset):
    """Two independent legal V8 samples with one shared augmentation draw."""
    def shared_augment(self, clip_a, clip_b):
        flip = random.random() < self.cfg.hflip_p
        lo, hi = self.cfg.crop_scale
        scale = random.uniform(lo, hi)
        side = int(round(self.cfg.img_size * scale))
        top = random.randint(0, self.cfg.img_size - side)
        left = random.randint(0, self.cfg.img_size - side)
        use_calibration = random.random() < self.cfg.calibration_p
        contrast = 1.0 + random.uniform(-self.cfg.contrast_delta, self.cfg.contrast_delta)
        brightness = random.uniform(-self.cfg.brightness_delta, self.cfg.brightness_delta)
        ...
        def apply(clip):
            if flip:
                clip = torch.flip(clip, [-1])
            clip = F.interpolate(
                clip[:, :, top:top + side, left:left + side],
        ...
        return apply(clip_a), apply(clip_b)

    def __getitem__(self, idx):
        assert self.train, 'TemporalPairDataset is training-only'
        indices_a = temporal_indices64('single', train=True)
        indices_b = temporal_indices64('single', train=True)
        if np.array_equal(indices_a, indices_b):
            # Preserve chronological order and pair membership while ensuring
            # that the consistency path is not accidentally an identical view.
            indices_b[0] = 1 - indices_b[0]
        source = self.mm[self.rows[idx]]
        clip_a = torch.from_numpy(np.asarray(source[indices_a]).copy()).float().div_(255.0)
        clip_b = torch.from_numpy(np.asarray(source[indices_b]).copy()).float().div_(255.0)
        clip_a, clip_b = self.shared_augment(clip_a, clip_b)
        clip_a = clip_a.sub_(KINETICS_MEAN).div_(KINETICS_STD)
        clip_b = clip_b.sub_(KINETICS_MEAN).div_(KINETICS_STD)
        return clip_a, clip_b, int(self.labels[idx]), idx
...
def mvit_logits_and_features(model, video):
    """Torchvision MViTv2 forward that also exposes the pre-head CLS feature."""
    x = model.conv_proj(video)
    thw = (model.pos_encoding.temporal_size, *model.pos_encoding.spatial_size)
    assert tuple(x.shape[2:]) == tuple(thw), (x.shape, thw)
...
def paired_video_mixup(clip_a, clip_b, label, cfg):
    """Apply exactly the same MixUp draw to both temporal samples."""
    assert clip_a.shape == clip_b.shape and len(clip_a) == len(label)
    targets = F.one_hot(label, N_CLASSES).float()
    targets = targets * (1.0 - cfg.label_smoothing) + cfg.label_smoothing / N_CLASSES
    mixed = False
    lam = 1.0
    if len(clip_a) > 1 and random.random() < cfg.mixup_p:
        lam = float(np.random.beta(cfg.mixup_alpha, cfg.mixup_alpha))
        lam = max(lam, 1.0 - lam)
        permutation = torch.randperm(len(clip_a), device=clip_a.device)
        clip_a = clip_a * lam + clip_a[permutation] * (1.0 - lam)
        clip_b = clip_b * lam + clip_b[permutation] * (1.0 - lam)
        targets = targets * lam + targets[permutation] * (1.0 - lam)
        mixed = True
    return clip_a, clip_b, targets, mixed, lam
...
def train_selection(meta, cfg):
    ...
    fp32_scores, _, _ = diagnostic_report(diagnostic_meta, fp32_probs, cfg,
        'v22_boundary_fp32', f'V22 native-160 EMA epoch {best_epoch}; held-out users {cfg.boundary_users}')
    ...
    scores, predictions, folder = diagnostic_report(diagnostic_meta, int8_probs, cfg,
        'v22_boundary_int8', f'V22 native-160 INT8 boundary epoch {best_epoch}; no full-training subjects')
    ...
    int8_acc = single_acc
    print(f'DIAGNOSTICS V22: FP32 single+hflip={best_acc:.4f}, INT8 single+hflip={single_acc:.4f}, '
    ...
    if champion_probs is not None:
        ...
        comparison.to_csv(cfg.out_dir / 'legacy_champion_vs_v22_boundary.csv', index=False)
    (cfg.out_dir / 'test_view_decision.json').write_text(json.dumps(dict(
        criterion='V22 diagnostics only; deployment remains fixed to V16 single+hflip',
...
def load_v16_model(cfg):
    ...
    if checkpoint.get('split') == 'boundary':
        raise ValueError('V22 refuses boundary-only V16 weights')
    ...
    print(f'V22 checkpoint QA: full-training V16, source={source_precision}, strict_state_load=True')
    return model
...
def consistency_weight_at(step, cfg):
    return cfg.consistency_weight * min((step + 1) / max(cfg.consistency_ramp_updates, 1), 1.0)


def train_temporal_consistency(meta, cfg):
    seed_everything(cfg.seed + 200)
    npy = cfg.cache_dir / 'train_thermal.npy'
    model = load_v16_model(cfg).to(DEVICE)
    loader = DataLoader(TemporalPairDataset(npy, meta, cfg, True), cfg.batch_size, shuffle=True,
                        drop_last=True, **common_loader_args(cfg))
    ema = ModelEmaV3(model, decay=cfg.ema_decay, device=DEVICE)
    ema.module.eval()
    ...
    step, history = 0, []
    for epoch in range(1, epochs + 1):
        model.train(); ema.module.eval(); optimizer.zero_grad(set_to_none=True)
        total_sum = ce_sum = consistency_sum = input_difference_sum = 0.0
        seen = mixed_microbatches = microbatches = 0
        start = time.time()
        remaining_updates = cfg.adaptation_updates - step
        usable_microbatches = min(updates_per_epoch, remaining_updates) * cfg.grad_accum
        for micro_step, (clip_a, clip_b, label, _) in enumerate(loader):
            ...
            clip_a, clip_b, soft_targets, mixed, _ = paired_video_mixup(
                clip_a, clip_b, label, cfg)
            input_difference = (clip_a.detach() - clip_b.detach()).abs().mean()
            with torch.no_grad(), torch.autocast('cuda', enabled=scaler.is_enabled()):
                _, teacher_features = mvit_logits_and_features(
                    ema.module, clip_b.permute(0, 2, 1, 3, 4))
            with torch.autocast('cuda', enabled=scaler.is_enabled()):
                logits, student_features = mvit_logits_and_features(
                    model, clip_a.permute(0, 2, 1, 3, 4))
                ce_loss = soft_target_cross_entropy(logits, soft_targets)
                student_unit = F.normalize(student_features.float(), dim=1, eps=1e-6)
                teacher_unit = F.normalize(teacher_features.float(), dim=1, eps=1e-6)
                consistency_loss = 1.0 - (student_unit * teacher_unit).sum(dim=1).mean()
                active_weight = consistency_weight_at(step, cfg)
                raw_loss = ce_loss + active_weight * consistency_loss
                loss = raw_loss / cfg.grad_accum
            if not all(torch.isfinite(value) for value in (ce_loss, consistency_loss, raw_loss)):
                raise FloatingPointError(f'Non-finite V22 loss at update {step}')
            if epoch == 1 and micro_step == 0:
                print(f'V22 pair QA: A/B={tuple(clip_a.shape)}, feature={tuple(student_features.shape)}, '
                      f'input_abs_difference={input_difference.item():.5f}, '
                      f'consistency={consistency_loss.item():.5f}, gate=none')
            scaler.scale(loss).backward()
            total_sum += raw_loss.item() * len(label)
            ce_sum += ce_loss.item() * len(label)
            consistency_sum += consistency_loss.item() * len(label)
            input_difference_sum += input_difference.item() * len(label)
        ...
        history.append(dict(epoch=epoch, optimizer_updates=step,
                            ...
                            consistency_weight=consistency_weight_at(max(step - 1, 0), cfg),
                            total_loss=total_sum / max(seen, 1),
                            classification_loss=ce_sum / max(seen, 1),
                            latent_consistency_loss=consistency_sum / max(seen, 1),
                            input_abs_difference=input_difference_sum / max(seen, 1),
                            mixup_microbatch_fraction=mixed_microbatches / max(microbatches, 1),
                            seconds=time.time()-start))
        pd.DataFrame(history).to_csv(cfg.out_dir / 'temporal_consistency_history.csv', index=False)
        print(f'V22 adaptation {epoch:02d}/{epochs} updates={step}/{cfg.adaptation_updates} '
              f'lr={optimizer.param_groups[0]["lr"]:.2e} '
              f'ce={ce_sum/max(seen,1):.4f} consistency={consistency_sum/max(seen,1):.4f} '
              f'total={total_sum/max(seen,1):.4f} time={time.time()-start:.0f}s')
...
def predict_test(model_path, meta, cfg):
    ...
    # V22 official inference is strictly the V16 single temporal view + hflip.
    if DEVICE.type == 'cuda':
    ...
    print(f"V22 test inference: {inference_seconds:.2f}s total, "
...
selected_view = 'single'
print('V22 begins short adaptation directly: boundary selection and duplicate MaskFeat are skipped.')
final_state = train_temporal_consistency(train_meta, CFG)
packed = quantize_state_dict(final_state)
FINAL_MODEL = CFG.out_dir / "thermal_v22_mvitv2s_160_temporal_latent_consistency_int8.pt"
torch.save({
    "model_int8": packed,
    "architecture": "torchvision_mvit_v2_s_relative_position_32x160",
    ...
    "pretraining": "inherited V16 Kinetics plus CUHK Thermal MaskFeat representation; not rerun in V22",
    "experiment_hypothesis": "EMA latent consistency across two independent legal V8 temporal samples",
    ...
    "consistency_type": "ungated cosine distance on normalized pre-head CLS features",
    "consistency_weight": CFG.consistency_weight,
    "consistency_ramp_updates": CFG.consistency_ramp_updates,
    "teacher": "same EMA used for final deployment; second temporal view; no gradient",
    "paired_augmentation": "shared crop, flip, calibration, MixUp partner and lambda",
...
print('V22 saved its final deployment model before deleting the reproducible training frame cache.')
...
if train_frame_cache.exists():
    ...
    print(f'V22 disk guard: removed reproducible train frame cache ({released_gib:.2f} GiB) before test caching.')
gc.collect()
