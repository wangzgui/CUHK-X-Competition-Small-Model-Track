...
class Config:
    ...
    # By default V20 searches every mounted /kaggle/input dataset for the exact
    ...
    maskfeat_final_grid: tuple[int, int, int] = (16, 6, 6)

    # Short, low-LR resolution transfer from the already trained V16-160 model.
    progressive_updates: int = 935
    progressive_lr: float = 2e-5
    progressive_min_lr: float = 1e-6
    progressive_warmup_steps: int = 47
    ...
    @property
    def cache_dir(self):
        return self.cache_root / f"V20_Tcache{self.cache_frames}_S{self.img_size}_IRcrop_uniform_v1"

    @property
    def out_dir(self):
        return self.work_root / "thermal_v20_mvitv2s_160to192_progressive_finetune"
...
def locate_v16_checkpoint(cfg):
    ...
    if not candidates:
        raise FileNotFoundError(
            'V20 requires the FULL-TRAINING V16 model before any cache is built. '
    ...
    if 'DIAGNOSTIC_ONLY' in selected.name or 'boundary' in selected.name.lower():
        raise ValueError(f'Boundary-only checkpoint is forbidden for V20: {selected}')
    return selected


CFG = Config()
assert (CFG.cache_frames, CFG.model_frames, CFG.img_size, CFG.batch_size, CFG.grad_accum) == (64, 32, 192, 2, 6)
assert CFG.maskfeat_final_grid == (16, 6, 6) and CFG.maskfeat_hog_bins == 9
assert (CFG.progressive_updates, CFG.progressive_lr, CFG.progressive_warmup_steps) == (935, 2e-5, 47)
...
assert "S192" in CFG.cache_dir.name and "S160" not in CFG.cache_dir.name and "S128" not in CFG.cache_dir.name
...
assert CFG.batch_size >= 2, "V20 requires physical batch >=2 so V16 Video MixUp remains active"
...
print('V20: short 160->192 progressive resolution fine-tune from the completed V16 model')
print(f'Progressive fine-tune: updates={CFG.progressive_updates}, lr={CFG.progressive_lr:.1e}, '
      f'min_lr={CFG.progressive_min_lr:.1e}, warmup={CFG.progressive_warmup_steps}')
print('Single hypothesis: preserve V16 while adapting its complete representation to native 192 spatial sampling.')
print('Cache guard: raw Thermal -> fixed IR union crop (margin=1.60) -> direct 192x192 resize; S128/S160 caches are forbidden.')
print('No boundary training, no MaskFeat rerun, no classifier reinitialization, and no V19 Linear Probe.')
...
def build_cache(clips, windows, split, cfg):
    ...
    signature_data = dict(version='v20_raw_thermal_direct192_uniform64', shape=shape,
...
if stale_test_frame_cache.exists():
    ...
    print(f'V20 disk guard: removed reproducible stale test frame cache ({released_gib:.2f} GiB) before training.')
...
print('V20 disk guard: defer the 192x192 test cache until training finishes; train and test frame caches are never resident together.')
...
def build_model(pretrained, cfg):
    ...
    # metadata to our actual post-convolution grid (16x48x48 for 32x192 input).
...
def thermal_hog_targets(normalized_clip, cfg):
    """Return L2-normalized 9-bin unsigned HOG on the final 16x6x6 token grid."""
...
assert (probe_model.pos_encoding.temporal_size, *probe_model.pos_encoding.spatial_size) == (16, 48, 48)
probe_model.eval()
with torch.no_grad():
...
print(f"MViTv2-S 32x192 forward preflight: output={tuple(probe_output.shape)} OK")
del probe_output
del probe_model
...
def train_selection(meta, cfg):
    ...
    fp32_scores, _, _ = diagnostic_report(diagnostic_meta, fp32_probs, cfg,
        'v18_boundary_fp32', f'V18 native-192 EMA epoch {best_epoch}; held-out users {cfg.boundary_users}')
    ...
    torch.save(dict(model_int8=packed, classes=LABEL_VALUES.tolist(), split='boundary',
                    ...
                    architecture='torchvision_mvit_v2_s_relative_position_32x192'),
    ...
    scores, predictions, folder = diagnostic_report(diagnostic_meta, int8_probs, cfg,
        'v18_boundary_int8', f'V18 native-192 INT8 boundary epoch {best_epoch}; no full-training subjects')
    ...
    int8_acc = single_acc
    print(f'DIAGNOSTICS V18: FP32 single+hflip={best_acc:.4f}, INT8 single+hflip={single_acc:.4f}, '
    ...
    if champion_probs is not None:
        ...
        comparison.to_csv(cfg.out_dir / 'legacy_champion_vs_v18_boundary.csv', index=False)
    (cfg.out_dir / 'test_view_decision.json').write_text(json.dumps(dict(
        criterion='V18 isolates native 192 sampling; deployment remains fixed to V16 single+hflip',
...
def load_v16_model_for_192(cfg):
    ...
    if checkpoint.get('split') == 'boundary':
        raise ValueError('V20 refuses boundary-only V16 weights')
    ...
    print(f'V20 checkpoint QA: full-training V16, source={source_precision}, strict_state_load=True')
    return model


def progressive_lr_at(step, cfg):
    if step < cfg.progressive_warmup_steps:
        return cfg.progressive_lr * (step + 1) / max(cfg.progressive_warmup_steps, 1)
    progress = (step - cfg.progressive_warmup_steps) / max(
        cfg.progressive_updates - cfg.progressive_warmup_steps, 1)
    progress = min(max(progress, 0.0), 1.0)
    return cfg.progressive_min_lr + .5 * (cfg.progressive_lr - cfg.progressive_min_lr) * (
        1 + math.cos(math.pi * progress))


def train_progressive_192(meta, cfg):
    seed_everything(cfg.seed + 200)
    npy = cfg.cache_dir / 'train_thermal.npy'
    model = load_v16_model_for_192(cfg).to(DEVICE)
    loader = DataLoader(ThermalDataset(npy, meta, cfg, True), cfg.batch_size, shuffle=True,
                        drop_last=True, **common_loader_args(cfg))
    ema = ModelEmaV3(model, decay=cfg.ema_decay, device=DEVICE)
    optimizer = torch.optim.AdamW(model.parameters(), lr=cfg.progressive_lr,
                                  weight_decay=cfg.weight_decay)
    scaler = torch.amp.GradScaler('cuda', enabled=cfg.amp and DEVICE.type == 'cuda')
    updates_per_epoch = len(loader) // cfg.grad_accum
    assert updates_per_epoch > 0
    epochs = math.ceil(cfg.progressive_updates / updates_per_epoch)
    step, history = 0, []
    for epoch in range(1, epochs + 1):
        model.train(); optimizer.zero_grad(set_to_none=True)
        loss_sum = seen = 0; start = time.time()
        remaining_updates = cfg.progressive_updates - step
        usable_microbatches = min(updates_per_epoch, remaining_updates) * cfg.grad_accum
        for micro_step, (clip, label, _) in enumerate(loader):
            if micro_step >= usable_microbatches or step >= cfg.progressive_updates:
                break
            clip, label = clip.to(DEVICE, non_blocking=True), label.to(DEVICE, non_blocking=True)
            clip, soft_targets = video_mixup(clip, label, cfg)
            with torch.autocast('cuda', enabled=scaler.is_enabled()):
            ...
            if not torch.isfinite(raw_loss):
                raise FloatingPointError(f'Non-finite V20 loss at update {step}')
            scaler.scale(loss).backward()
            loss_sum += raw_loss.item() * len(label); seen += len(label)
            if (micro_step + 1) % cfg.grad_accum == 0:
                for group in optimizer.param_groups:
                    group['lr'] = progressive_lr_at(step, cfg)
                scaler.unscale_(optimizer)
                nn.utils.clip_grad_norm_(model.parameters(), cfg.grad_clip)
                scaler.step(optimizer); scaler.update(); optimizer.zero_grad(set_to_none=True)
                ema.update(model, step=step); step += 1
        history.append(dict(epoch=epoch, optimizer_updates=step,
                            ...
                            train_loss=loss_sum / max(seen, 1), seconds=time.time()-start))
        pd.DataFrame(history).to_csv(cfg.out_dir / 'progressive_192_history.csv', index=False)
        print(f'V20 progressive {epoch:02d}/{epochs} updates={step}/{cfg.progressive_updates} '
              f'lr={optimizer.param_groups[0]["lr"]:.2e} '
              f'loss={loss_sum/max(seen,1):.4f} time={time.time()-start:.0f}s')
    assert step == cfg.progressive_updates, (step, cfg.progressive_updates)
...
def predict_test(model_path, meta, cfg):
    ...
    # V20 official inference is strictly the V16 single temporal view + hflip.
    if DEVICE.type == 'cuda':
    ...
    print(f"V20 test inference: {inference_seconds:.2f}s total, "
...
selected_view = 'single'
print('V20 begins deployment training directly: boundary selection and duplicate MaskFeat are skipped.')
final_state = train_progressive_192(train_meta, CFG)
packed = quantize_state_dict(final_state)
FINAL_MODEL = CFG.out_dir / "thermal_v20_mvitv2s_160to192_progressive_int8.pt"
torch.save({
    "model_int8": packed,
    "architecture": "torchvision_mvit_v2_s_relative_position_32x192",
    ...
    "pretraining": "inherited V16 Kinetics plus CUHK Thermal MaskFeat representation; not rerun in V20",
    "experiment_hypothesis": "short low-LR transfer of the successful V16 representation to native 192 sampling",
    "cache_preprocessing": "raw Thermal -> fixed IR union crop margin 1.60 -> direct 192x192 resize",
    "pretraining_test_data_used": False,
    "image_size": CFG.img_size,
    "input_channels": CFG.in_channels,
    "progressive_updates": CFG.progressive_updates,
    "progressive_lr": CFG.progressive_lr,
    "progressive_min_lr": CFG.progressive_min_lr,
    "progressive_warmup_steps": CFG.progressive_warmup_steps,
...
print('V20 saved its final deployment model before deleting the reproducible training frame cache.')

# A simultaneous 192x192 train+test uint8 cache would require about 21.7 GiB.
...
if train_frame_cache.exists():
    ...
    print(f'V20 disk guard: removed reproducible train frame cache ({released_gib:.2f} GiB) before test caching.')
gc.collect()
