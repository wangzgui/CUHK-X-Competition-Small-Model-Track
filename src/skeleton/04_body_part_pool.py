...
OUT_DIR = Path("/kaggle/working/skeleton_body_part_pool")
SUBMISSION_PATH = Path("/kaggle/working/submission_body_part_pool.csv")
AUDIT_ONLY = False
# Avoid spending extra time on two final models when the five-fold experiment
# has already failed to match the verified baseline.
MIN_CV_FOR_FINAL = 0.4476
...
class BodyPartPoolingHead(nn.Module):
    """Retain anatomical location instead of averaging all 17 joints together."""
    PARTS = (
        tuple(range(17)),       # global context
        (0, 1, 2, 3, 4),       # head
        (5, 7, 9),             # left arm
        (6, 8, 10),            # right arm
        (5, 6, 11, 12),        # torso
    ...
    def __init__(self, channels, classes, drop=.25):
        super().__init__()
        mask = torch.zeros(len(self.PARTS), 17, dtype=torch.float32)
        for part, joints in enumerate(self.PARTS):
            mask[part, list(joints)] = 1.0 / len(joints)
        self.register_buffer("part_mask", mask)
        self.drop = nn.Dropout(drop)
        # 44,840 parameters for 160 channels and 40 classes: under 0.1 MB
        # in the exported FP16 checkpoint.
        self.fc = nn.Linear(channels * len(self.PARTS), classes)

    def forward(self, x):
        # (N,C,T,V) -> joint-weighted parts -> temporal mean -> (N,7C).
        parts = torch.einsum("nctv,pv->ncpt", x, self.part_mask).mean(dim=-1)
        parts = parts.permute(0, 2, 1).flatten(1)
        return self.fc(self.drop(parts))
...
class SmallSTGCN(nn.Module):
    def __init__(self, classes):
        ...
        self.head = BodyPartPoolingHead(160, classes)
    def forward(self, x):
        n, c, t, v = x.shape
...
def main():
    ...
    oof_path = analysis / "oof_body_part_pool.csv"
    ...
    if AUDIT_ONLY:
        ...
        return
    if np.mean(scores) < MIN_CV_FOR_FINAL:
        print(f"CV {np.mean(scores):.4f} is below the {MIN_CV_FOR_FINAL:.4f} baseline; "
              "saved OOF and skipped final-model/test work.")
        return
