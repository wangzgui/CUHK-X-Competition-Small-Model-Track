...
OUT_DIR = Path("/kaggle/working/skeleton_ema")
SUBMISSION_PATH = Path("/kaggle/working/submission_ema.csv")
AUDIT_ONLY = False
...
CFG = dict(seed=2026, frames=48, batch_size=64, epochs=45, warmup_epochs=4,
           ...
           use_multiscale_tcn=True, ema_decay=.995,
...
class ModelEMA:
    ...
    def update(self, model):
        self.updates += 1
        # A short automatic warm-up prevents the randomly initialised first
        # batches from dominating a high fixed-decay average.
...
def train_one_fold(train_x, train_y, val_x, val_y, n_classes, fold):
    ...
    for epoch in range(CFG["epochs"]):
        ...
        for x, y in train_loader:
            ...
            ema.update(model)
            step += 1
        raw_p = predict(model, val_loader); raw_acc = (raw_p.argmax(1) == val_y).mean()
        p = predict(ema.module, val_loader); acc = (p.argmax(1) == val_y).mean()
        if acc > best:
            best, best_epoch = acc, epoch + 1
            best_sd = {k: v.detach().cpu().clone() for k, v in ema.module.state_dict().items()}
        print(f"fold {fold} epoch {epoch+1:02d}: raw_acc={raw_acc:.4f}, "
              f"ema_acc={acc:.4f}, best_ema={best:.4f}")
    ...
    ema.module.load_state_dict(best_sd)
    return best, best_epoch, predict(ema.module, val_loader)
...
def train_full(x, y, n_classes, epochs, model_id):
    ...
    sd = {k: v.detach().cpu().clone() for k, v in ema.module.state_dict().items()}
    size = save_compact(sd, n_classes, OUT_DIR / f"final_model_{model_id}.pt",
                        epochs=epochs, ema_decay=CFG["ema_decay"])
...
def main():
    ...
    if np.mean(scores) < MIN_CV_FOR_FINAL:
        print(f"EMA CV {np.mean(scores):.4f} is below the {MIN_CV_FOR_FINAL:.4f} "
              "baseline; saved OOF and skipped final-model/test work.")
        return
